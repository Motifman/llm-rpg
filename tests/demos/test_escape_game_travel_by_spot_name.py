"""Issue #227 PR 3: travel_to の display_name fallback を escape_game 経路でも有効化する E2E テスト。
...
display_name (= スポット名) fallback を追加したが、escape_game runtime は
独自に ``_execute_tool`` で直接 label lookup していたため、その修正は
escape_game 経路では効いていなかった。

本 PR 3 で escape_game の ``_execute_tool`` にも同等の fallback を追加し、
