...
**段階的進め方**（依存関係を考慮した推奨順）:
1. Phase B3a: `PlayerLocationQueryService` を切り出し（単独で切り出しやすい）
2. Phase B3a': `PlayerAudienceQueryService` の world 系利用。SpotContext と PlayerCurrentStateBuilder の「同スポット人数」取得を `players_at_spot` に統一
