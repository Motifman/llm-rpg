...
**自分の注文を除いた最安・最高**が「その人に買える値」であること。そして
...
class TestTheRowsSummariseEachItem:
    ...
    def test_the_row_counts_the_orders_and_the_goods(self) -> None:
        """行は、売り注文の件数と総数量を持つ。
        ...
        assert row.sell_count == 3
        assert row.sell_quantity == 9

    def test_the_row_shows_the_cheapest_sell_and_the_highest_buy(self) -> None:
        """行は、売りの最安と買いの最高を持つ。

        この 2 つが並ぶことで需給が見える。値が交差していれば、そのまま
        **機会**として読める (engine は交差を潰さない)。
        """
        board = _board(
        ...
        assert row.best_sell_price == 18
        assert row.best_buy_price == 15

    def test_a_side_with_no_orders_has_no_price(self) -> None:
        """片側に注文が無ければ、その値は無い (0 ではない)。

        0 を入れると「0G で買い注文が出ている」と読めてしまう。
        """
        board = _board(_order(1, side=MarketOrderSide.SELL))

        row = board.rows_for(_TOM).rows[0]

        assert row.best_buy_price is None
...
class TestThePricesShownAreThePricesYouCanTake:
    """見える最安・最高は、その人が実際に受けられる値になる。"""

    def test_your_own_order_is_not_counted_as_the_best_price(self) -> None:
        """自分の注文は最安に数えない。

        自分の注文は自分で受けられないので、自分の 18G を「最安」として
        見せると、買えない値を相場として読むことになる。
        """
        board = _board(
        ...
        assert board.rows_for(_LENA).rows[0].best_sell_price == 20

    def test_someone_else_sees_the_cheaper_one(self) -> None:
        """他人から見れば、その 18G が最安のまま (正の対照)。"""
        board = _board(
            _order(1, owner=_LENA, unit_price_gold=18),
            _order(2, owner=_TOM, unit_price_gold=20),
        )

        assert board.rows_for(_TOM).rows[0].best_sell_price == 18

    def test_a_row_of_only_your_own_orders_has_no_takeable_price(self) -> None:
        """自分の注文しか無い品目は、受けられる値が無い。"""
        board = _board(_order(1, owner=_LENA, unit_price_gold=18))

        row = board.rows_for(_LENA).rows[0]

        assert row.best_sell_price is None
