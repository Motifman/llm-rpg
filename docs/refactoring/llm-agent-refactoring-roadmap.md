...
- **残作業**: 13 項目（Phase 1 が 1 件、Phase 2 が 4 件、Phase 3 が 8 件）
- **並列化可能グループ**:
  - **LLM/Tool 系**（2.1, 2.2, 3.7, 3.8）: application/llm 配下。2.1 と 2.2 は ToolCommandMapper 周りでやや関連するが、3.7/3.8 は独立して着手可能
  - **World 系**（1.1, 2.3, 2.4, 3.4）: 世界シミュレーション・マップ・移動・クエリ。2.3 → 2.4 の順が望ましい
  - **Observation 系**（3.1, 3.2, 3.3）: application/observation 配下。recipient strategy や handler が共通で絡むため、同一担当者がまとめて着手するのが効率的
  - **Domain 系**（3.5, 3.6）: Monster / Player の aggregate。ドメイン境界が明確に分かれており、完全に並列化可能
- **次に着手**: 2.1 ToolArgumentResolver（ロードマップ推奨順）
