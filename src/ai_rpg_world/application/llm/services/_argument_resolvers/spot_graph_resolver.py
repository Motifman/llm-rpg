...
_SPOT_GRAPH_TOOLS = frozenset({
    ...
    TOOL_NAME_SPOT_GRAPH_USE_ITEM,
    TOOL_NAME_SPOT_GRAPH_GIVE_ITEM,
    # PR-BB (Y_after_pr639_640 後続): TOOL_NAME_SPOT_GRAPH_GIVE_ITEMS は
    # 削除された。PR-W の dispatch bug 修正はもう対象 tool が無いので
    # 意味を失っているが、gate 網羅性の regression test は残す。
    TOOL_NAME_SPOT_GRAPH_TEND_TO_PLAYER,
})
...
class SpotGraphArgumentResolver:
    ...
    # PR-BB (Y_after_pr639_640 後続): 旧 ``_resolve_give_items`` (batch 版)
    # は削除。give_items tool は spec ごと除去。将来 batch を戻すときは
    # ``give_item`` の union 型拡張として実装する。
