...
from ai_rpg_world.application.llm.contracts.interfaces import IActionResultStore, IAvailableToolsProvider, ICurrentStateFormatter, ILlmUiContextBuilder, IRecentEventsFormatter, IShortTermMemory, ISystemPromptBuilder
...
from ai_rpg_world.application.llm.services.unified_recent_event_store import UnifiedRecentEventStore
from ai_rpg_world.application.llm.services.prompt_builder_config import EpisodicRecallConfig, PromptBuilderCoreServices
...
def _profile_repo(player_id: int=1) -> PlayerProfileRepository:
    ...
    profile = PlayerProfileAggregate.create(PlayerId(player_id), PlayerName('PromptTest'), control_type=ControlType.LLM)
    repo.save(profile)
    return repo

def _episode(*, episode_id: str, player_id: int, occurred_at: datetime, recall_text: str, cues: tuple[EpisodicCue, ...], game_time_label: str | None=None) -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=episode_id, player_id=player_id, being_id=BeingId(f'being_w1_p{player_id}'), occurred_at=occurred_at, game_time_label=game_time_label, source=EpisodeSource(event_ids=('evt-x',)), location=EpisodeLocation(), action=EpisodeAction(tool_name='t'), who=('p',), what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=cues, recall_text=recall_text)
...
class TestPromptBuilderPassiveRecall:
    ...
    def test_without_service_relevant_memories_section_is_omitted(self) -> None:
        ...
        ui_builder.build = MagicMock(return_value=LlmUiContextDto(current_state_text='ui', tool_runtime_context=ToolRuntimeContextDto.empty()))
        builder = DefaultPromptBuilder(PromptBuilderCoreServices(observation_buffer=buffer, short_term_memory=sliding, action_result_store=actions, recent_event_store=UnifiedRecentEventStore(), world_query_service=world, player_profile_repository=_profile_repo(), current_state_formatter=current_fmt, recent_events_formatter=recent_fmt, context_format_strategy=SectionBasedContextFormatStrategy(), system_prompt_builder=sys_builder, available_tools_provider=tools_p), ui_context_builder=ui_builder)
        out = builder.build(ActingBeing(player_id=PlayerId(1), being_id=being_id))
    ...
    def test_with_service_joins_recall_texts_into_related_memories_section(self) -> None:
        ...
        being_id_3 = BeingProvisioningService(_being_repo).ensure_attached(PlayerId(player_num))
        ...
        store.put_by_being(being_id_3, _episode(episode_id='e_recent', player_id=player_num, occurred_at=base + timedelta(hours=1), recall_text='最近の出来事', cues=(place_c,), game_time_label='Day 2 09:00'))
        store.put_by_being(being_id_3, _episode(episode_id='e_old', player_id=player_num, occurred_at=base, recall_text='古いが cue で拾える', cues=(place_c,)))
        recall_svc = EpisodicPassiveRecallRetrievalService(store)
        buffer = MagicMock(spec=IObservationContextBuffer)
        buffer.drain = MagicMock(return_value=[])
        sliding = MagicMock(spec=IShortTermMemory)
        sliding.append_all = MagicMock(return_value=[])
        sliding.get_recent = MagicMock(return_value=[ObservationEntry(occurred_at=base + timedelta(hours=2), output=ObservationOutput(prose='look', structured={'spot_id_value': 77}, observation_category='environment'), game_time_label=None)])
        actions = MagicMock(spec=IActionResultStore)
        actions.get_recent = MagicMock(return_value=[])
        world = _world_query_stub(state=None)
        current_fmt = MagicMock(spec=ICurrentStateFormatter)
        ...
        ui_builder = MagicMock(spec=ILlmUiContextBuilder)
        ui_builder.build = MagicMock(return_value=LlmUiContextDto(current_state_text='ui', tool_runtime_context=rt))
        builder = DefaultPromptBuilder(PromptBuilderCoreServices(observation_buffer=buffer, short_term_memory=sliding, action_result_store=actions, recent_event_store=UnifiedRecentEventStore(), world_query_service=world, player_profile_repository=_profile_repo(player_id=player_num), current_state_formatter=current_fmt, recent_events_formatter=recent_fmt, context_format_strategy=SectionBasedContextFormatStrategy(), system_prompt_builder=sys_builder, available_tools_provider=tools_p), ui_context_builder=ui_builder, episodic=EpisodicRecallConfig(passive_recall=recall_svc, passive_recall_limit_per_axis=_TEST_PASSIVE_RECALL_LIMIT_PER_AXIS, passive_recall_max_candidates=_TEST_PASSIVE_RECALL_MAX_CANDIDATES))
        out = builder.build(ActingBeing(player_id=PlayerId(player_num), being_id=being_id_3))
        user = out['messages'][1]['content']
    ...
    def test_u1_prediction_context_id_recall_episode_recall_observation_stamp(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.prediction_context_ledger import PredictionContextLedger
        ...
        base = datetime(2026, 5, 2, 8, 0, tzinfo=timezone.utc)
        store = InMemorySubjectiveEpisodeStore()
        store.put_by_being(being_id_5, _episode(episode_id='e_for_ledger', player_id=player_num, occurred_at=base, recall_text='ledger 紐付け確認用', cues=(place_c,)))
        recall_svc = EpisodicPassiveRecallRetrievalService(store)
        buffer = MagicMock(spec=IObservationContextBuffer)
        buffer.drain = MagicMock(return_value=[])
        sliding = MagicMock(spec=IShortTermMemory)
        sliding.append_all = MagicMock(return_value=[])
        sliding.get_recent = MagicMock(return_value=[ObservationEntry(occurred_at=base + timedelta(hours=1), output=ObservationOutput(prose='look', structured={'spot_id_value': 77}, observation_category='environment'), game_time_label=None)])
        actions = MagicMock(spec=IActionResultStore)
        actions.get_recent = MagicMock(return_value=[])
        world = _world_query_stub(state=None)
        current_fmt = MagicMock(spec=ICurrentStateFormatter)
        ...
        builder = DefaultPromptBuilder(PromptBuilderCoreServices(observation_buffer=buffer, short_term_memory=sliding, action_result_store=actions, recent_event_store=UnifiedRecentEventStore(), world_query_service=world, player_profile_repository=_profile_repo(player_id=player_num), current_state_formatter=current_fmt, recent_events_formatter=recent_fmt, context_format_strategy=SectionBasedContextFormatStrategy(), system_prompt_builder=sys_builder, available_tools_provider=tools_p), ui_context_builder=ui_builder, episodic=EpisodicRecallConfig(passive_recall=recall_svc, passive_recall_limit_per_axis=_TEST_PASSIVE_RECALL_LIMIT_PER_AXIS, passive_recall_max_candidates=_TEST_PASSIVE_RECALL_MAX_CANDIDATES, recall_buffer_store=setup.recall_buffer, turn_index_provider=lambda _pid: 12), prediction_context_ledger=ledger)
        out = builder.build(ActingBeing(player_id=PlayerId(player_num), being_id=being_id_5))
        ...
        observations = setup.recall_buffer.peek_batch_by_being(being_id_5, batch_size=8, max_contexts_per_episode=3)
        assert len(observations) == 1
        assert observations[0].episode_id == 'e_for_ledger'
        assert observations[0].prediction_context_id == issued_id

    def test_u1_flag_off_ledger_uninjected_recall_observation_id_none(self) -> None:
        ...
        base = datetime(2026, 5, 2, 8, 0, tzinfo=timezone.utc)
        store = InMemorySubjectiveEpisodeStore()
        store.put_by_being(being_id_6, _episode(episode_id='e_no_id', player_id=player_num, occurred_at=base, recall_text='id 無し確認用', cues=(place_c,)))
        recall_svc = EpisodicPassiveRecallRetrievalService(store)
        buffer = MagicMock(spec=IObservationContextBuffer)
        buffer.drain = MagicMock(return_value=[])
        sliding = MagicMock(spec=IShortTermMemory)
        ...
        ui_builder.build = MagicMock(return_value=LlmUiContextDto(current_state_text='ui', tool_runtime_context=rt))
        builder = DefaultPromptBuilder(PromptBuilderCoreServices(observation_buffer=buffer, short_term_memory=sliding, action_result_store=actions, recent_event_store=UnifiedRecentEventStore(), world_query_service=world, player_profile_repository=_profile_repo(player_id=player_num), current_state_formatter=current_fmt, recent_events_formatter=recent_fmt, context_format_strategy=SectionBasedContextFormatStrategy(), system_prompt_builder=sys_builder, available_tools_provider=tools_p), ui_context_builder=ui_builder, episodic=EpisodicRecallConfig(passive_recall=recall_svc, passive_recall_limit_per_axis=_TEST_PASSIVE_RECALL_LIMIT_PER_AXIS, passive_recall_max_candidates=_TEST_PASSIVE_RECALL_MAX_CANDIDATES, recall_buffer_store=setup.recall_buffer, turn_index_provider=lambda _pid: 12))
        out = builder.build(ActingBeing(player_id=PlayerId(player_num), being_id=being_id_6))
        assert out['prediction_context_id'] is None
        observations = setup.recall_buffer.peek_batch_by_being(being_id_6, batch_size=8, max_contexts_per_episode=3)
        assert len(observations) == 1
    ...
    def test_active_reinterpretation_overrides_episode_recall_and_records_observation(self) -> None:
        ...
        store.put_by_being(being_id_4, _episode(episode_id='e_reinterpreted', player_id=player_num, occurred_at=base + timedelta(hours=1), recall_text='古い回想は使われない', cues=(place_c,)))
        recall_svc = EpisodicPassiveRecallRetrievalService(store)
        being_id = being_id_4
        ...
        rt = ToolRuntimeContextDto(targets={}, current_spot_id=77)
        ui_builder = MagicMock(spec=ILlmUiContextBuilder)
        ui_builder.build = MagicMock(return_value=LlmUiContextDto(current_state_text='ui-current', tool_runtime_context=rt))
        builder = DefaultPromptBuilder(PromptBuilderCoreServices(observation_buffer=buffer, short_term_memory=sliding, action_result_store=actions, recent_event_store=UnifiedRecentEventStore(), world_query_service=world, player_profile_repository=_profile_repo(player_id=player_num), current_state_formatter=current_fmt, recent_events_formatter=recent_fmt, context_format_strategy=SectionBasedContextFormatStrategy(), system_prompt_builder=sys_builder, available_tools_provider=tools_p), ui_context_builder=ui_builder, episodic=EpisodicRecallConfig(passive_recall=recall_svc, passive_recall_limit_per_axis=_TEST_PASSIVE_RECALL_LIMIT_PER_AXIS, passive_recall_max_candidates=_TEST_PASSIVE_RECALL_MAX_CANDIDATES, recall_buffer_store=recall_buffer, reinterpretation_journal_store=journal, turn_index_provider=lambda _pid: 12))
        out = builder.build(ActingBeing(player_id=PlayerId(player_num), being_id=being_id_4))
        user = out['messages'][1]['content']
        assert '私はあの場で感じた違和感' in user
        assert '古い回想は使われない' not in user
        pending = recall_buffer.peek_batch_by_being(being_id, batch_size=8, max_contexts_per_episode=3)
