...
**段階的進め方**:
1. Phase B1a: `MapTriggerEngine` を切り出し、`move_object` と `add_object` から委譲
2. Phase B1b: `HarvestSessionDomainService` を切り出し、採取ロジックを移行
3. Phase B1c: `MapInteractionPolicy` と `ChestInteractionPolicy` を切り出し、`interact_with` を整理
