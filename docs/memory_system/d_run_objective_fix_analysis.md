...
> 🔄 **逆訂正 (2026-06-13 夜)**: 2026-06-13 昼の訂正自体が誤りだった。私
> (Claude) の分析スクリプトが trace の存在しない key `cached_prompt_tokens`
> を見ていたため C run v3 の cache hit を 0% と誤判定していた。正しい key
> `cached_tokens` で再 grep すると **C run v3: 48.0% / D run: 0% / H run: 0%**
> が真値。**「DeepInfra で cache が消えた」結論は (D run については) 正しい**。
...
| 総 cached tokens | **0** | **1,802,545** |
| **cache hit rate** | **0.0%** | **48.0%** |

= **DeepInfra gemma fp8 は OpenRouter 経由で prefix cache が機能していない**。
Parasail gemma fp8 (C run v3) は 48% cache hit していたので、provider 依存。
これが D run の per-call latency 増加の主因の 1 つ。
