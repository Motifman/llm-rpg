...
import pytest
from ai_rpg_world.application.llm.services.afterglow_store import AfterglowEntry, AfterglowSource, InMemoryAfterglowStore, apply_afterglow_policy, make_afterglow_handle
from ai_rpg_world.domain.being.value_object.being_id import BeingId

def _entry(episode_id: str='e1', heading: str='h', entered_tick: int=0, source: AfterglowSource=AfterglowSource.SLOT_EVICTED) -> AfterglowEntry:
    return AfterglowEntry(episode_id=episode_id, heading=heading, entered_tick=entered_tick, source=source)
...
class TestAfterglowEntryValidation:
    ...
    def test_entry_holds_all_four_fields(self) -> None:
        ...
        e = AfterglowEntry(episode_id='ep-1', heading='司書の手記', entered_tick=3, source=AfterglowSource.SLOT_EVICTED)
...
class TestApplyAfterglowPolicy:
    ...
    def test_empty_index_accepts_a_new_entry(self) -> None:
        ...
        result = apply_afterglow_policy(prev_index=(), incoming=(new_entry,), current_tick=0, capacity=10, max_residence=10)
    ...
    def test_entry_older_than_max_residence_is_dropped(self) -> None:
        ...
        result = apply_afterglow_policy(prev_index=(old,), incoming=(), current_tick=10, capacity=10, max_residence=10)
        assert result == ()
    ...
    def test_exceeds_capacity_drops_oldest_first(self) -> None:
        ...
        result = apply_afterglow_policy(prev_index=(old1, old2), incoming=(new,), current_tick=2, capacity=2, max_residence=10)
    ...
    def test_reinsertion_updates_entered_tick(self) -> None:
        ...
        result = apply_afterglow_policy(prev_index=(old,), incoming=(re_entered,), current_tick=5, capacity=10, max_residence=10)
        assert len(result) == 1
...
class TestInMemoryAfterglowStore:
    ...
    def test_beings_are_isolated(self) -> None:
        ...
        store.apply_decision(being_a, (_entry(episode_id='e1', heading='h'),))
...
class TestFormatAfterglowSection:
    ...
    def test_empty_index_returns_empty_string(self) -> None:
        ...
        無駄に膨らまないようにする。"""
        from ai_rpg_world.application.llm.services.prompt_builder import _format_afterglow_section
    ...
    def test_non_empty_index_renders_heading_with_handle(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.prompt_builder import _format_afterglow_section
        index = (AfterglowEntry(episode_id='3f2a7b8c-9d0e', heading='司書の手記', entered_tick=0, source=AfterglowSource.SLOT_EVICTED),)
...
class TestResolveEpisodeIdFromHandle:
    ...
    def test_strips_ep_prefix_and_returns_id_prefix(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.afterglow_store import resolve_episode_id_prefix_from_handle
...
class TestAfterglowStoreLookupAndRemoval:
    ...
    def test_find_by_handle_returns_matching_entry(self) -> None:
        ...
        store = InMemoryAfterglowStore()
        being = _being()
        entry = AfterglowEntry(episode_id='3f2a7b8c-9d0e-4f1a-aaaa', heading='司書の手記', entered_tick=0, source=AfterglowSource.WEAK_RECALL)
    ...
    def test_remove_drops_entry_from_index(self) -> None:
        ...
        store = InMemoryAfterglowStore()
        being = _being()
        entry = AfterglowEntry(episode_id='abcd1234-zz', heading='h', entered_tick=0, source=AfterglowSource.WEAK_RECALL)
        store.apply_decision(being, (entry,))
    ...
    def test_remove_unknown_episode_id_is_noop(self) -> None:
        ...
        store = InMemoryAfterglowStore()
        being = _being()
        store.remove(being, 'never-existed')
        assert store.get_index(being) == ()
