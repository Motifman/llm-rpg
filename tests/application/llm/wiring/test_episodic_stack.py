...
from ai_rpg_world.application.llm.wiring.episodic_stack import EpisodicStack, build_episodic_stack, build_scenario_noun_matcher, is_episodic_enabled, is_episodic_subjective_enabled
from ai_rpg_world.application.observation.services.observation_context_buffer import DefaultObservationContextBuffer

def _stub_graph(*, spots: dict[int, str], interior_objects: dict[int, list[tuple[int, str]]] | None=None) -> SimpleNamespace:
    ...
    for (sid, name) in spots.items():
        ...
        objs = tuple((SimpleNamespace(object_id=SimpleNamespace(value=oid), name=oname) for (oid, oname) in interior_objects.get(sid, [])))
        interior = SimpleNamespace(objects=objs)
        nodes[sid] = SimpleNamespace(spot_id=spot_id_obj, name=name, interior=interior)
...
def _stub_scenario(*, players: list[tuple[int, str]]) -> SimpleNamespace:
    ...
    spawns = [SimpleNamespace(player_id=pid, name=name) for (pid, name) in players]
    return SimpleNamespace(player_spawns=spawns)

class TestIsEpisodicEnabled:
    """``LLM_EPISODIC_ENABLED`` の解釈 (シナリオ非依存)。"""

    @pytest.mark.parametrize('raw, expected', [('1', True), ('true', True), ('YES', True), ('on', True), ('0', False), ('false', False), ('', False), ('garbage', False)])
...
class TestBuildScenarioNounMatcher:
    ...
    def test_world_object(self) -> None:
        ...
        する経路を足す。
        """
        graph = _stub_graph(spots={1: '入口広間', 2: '閲覧室'}, interior_objects={1: [(101, '案内板'), (102, '侵入者の手記')], 2: [(201, '見習い司書の覚書')]})
        scenario = _stub_scenario(players=[(10, 'カイト')])
        matcher = build_scenario_noun_matcher(scenario=scenario, graph=graph)
        obj_hits = matcher.find_in_text('案内板を読んだ')
        assert any((m.axis == 'object' and m.value == 'world_object_101' for m in obj_hits)), f'案内板が world_object_101 にマッチしない: {obj_hits}'
        obj_hits2 = matcher.find_in_text('見習い司書の覚書には印の順序が書いてあった')
        assert any((m.axis == 'object' and m.value == 'world_object_201' for m in obj_hits2))
...
class TestBuildEpisodicStack:
    ...
    def _minimal_io(self):
        return (DefaultObservationContextBuffer(), DefaultSlidingWindowMemory(), DefaultActionResultStore())
    ...
    def test_episodic_stack_four_element_built(self) -> None:
        ...
        stack = build_episodic_stack(scenario=scenario, graph=graph, observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store)
    ...
    def test_episode_store(self) -> None:
        ...
        stack = build_episodic_stack(scenario=scenario, graph=graph, observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store, episode_store=shared)
...
class TestReinterpretationOptIn:
    ...
    def test_default_off_reinterpretation_all_none(self) -> None:
        ...
        stack = build_episodic_stack(scenario=_stub_scenario(players=[(1, 't')]), graph=_stub_graph(spots={1: 'x'}), observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store)
    ...
    def test_creates_prompt_buffer_none_enabled_completion_none_coordinator(self) -> None:
        ...
        (obs_buf, sliding, action_store) = self._io()
        stack = build_episodic_stack(scenario=_stub_scenario(players=[(1, 't')]), graph=_stub_graph(spots={1: 'x'}), observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store, reinterpretation_enabled=True, reinterpretation_completion=None)
    ...
    def test_enabled_completion_prompt_buffer_non_none(self) -> None:
        ...
        class _StubCompletion(IEpisodicReinterpretationCompletionPort):
            ...
            def complete_episodic_reinterpretation_json(self, messages):
                return '{}'
        (obs_buf, sliding, action_store) = self._io()
        stack = build_episodic_stack(scenario=_stub_scenario(players=[(1, 't')]), graph=_stub_graph(spots={1: 'x'}), observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store, reinterpretation_enabled=True, reinterpretation_completion=_StubCompletion())
        assert stack.reinterpretation_coordinator is not None
...
class TestRecallHitBoostOptIn:
    ...
    def test_enabled_recall_success_store_chunk_coordinator(self) -> None:
        """flag ON で的中側 sidecar が構築され、stack が公開する。"""
        (obs_buf, sliding, action_store) = self._io()
        stack = build_episodic_stack(scenario=_stub_scenario(players=[(1, 't')]), graph=_stub_graph(spots={1: 'x'}), observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store, recall_hit_boost_enabled=True, recall_hit_boost_strength=2)
    ...
    def test_enabled_passive_recall_same_store_strength_wired(self) -> None:
        """enabled なら passive recall にも 同一store と strength が配線される。"""
        (obs_buf, sliding, action_store) = self._io()
        stack = build_episodic_stack(scenario=_stub_scenario(players=[(1, 't')]), graph=_stub_graph(spots={1: 'x'}), observation_buffer=obs_buf, short_term_memory=sliding, action_result_store=action_store, recall_hit_boost_enabled=True, recall_hit_boost_strength=3, recall_hit_boost_cap=1)
        assert stack.passive_recall._recall_success_store is stack.recall_success_store
        assert stack.passive_recall._hit_boost_strength == 3
        assert stack.passive_recall._hit_boost_cap == 1
...
class TestBackwardCompatAlias:
    ...
    def test_legacy_world_episodic_stack_episodic_stack_alias(self) -> None:
        ...
        from ai_rpg_world.application.world_runtime.world_episodic_wiring import EpisodicStack as NewName, WorldEpisodicStack as OldName
    ...
    def test_legacy_build_world_episodic_stack_build_episodic_stack_alias(self) -> None:
        ...
        from ai_rpg_world.application.world_runtime.world_episodic_wiring import build_episodic_stack as new_build, build_world_episodic_stack as old_build
        assert old_build is new_build

    def test_legacy_env_alias_via_works(self) -> None:
        """旧名 env ヘルパも alias 経由で動く。"""
        from ai_rpg_world.application.world_runtime.world_episodic_wiring import is_episodic_enabled, is_episodic_subjective_enabled
