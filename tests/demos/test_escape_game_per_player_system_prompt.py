"""Issue #264 第16回実験 fix: escape_game の per-player system prompt 統合の挙動確認。
...
class TestEscapeGamePerPlayerSystemPrompt:
    ...
    def test_runtime_builds_per_player_system_prompts_when_multiple_spawns(self) -> None:
        ...
        assert len(runtime._escape_llm_system_prompts_by_player_id) == 2
    ...
    def test_system_prompt_builder_adapter_returns_per_player_prompt(self) -> None:
        """EscapeGameSystemPromptBuilder.build() が player_info.player_name から
        正しい per-player prompt を引く。"""
        runtime = create_escape_game_runtime(_FORBIDDEN_LIBRARY)
        builder = EscapeGameSystemPromptBuilder(runtime)
        player_ids = runtime.get_player_ids()
