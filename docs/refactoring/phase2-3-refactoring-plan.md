...
**現状**:
- `world_query_service.py`: 約 430 行、コンストラクタで 17 個の依存（必須 5 + Optional 12）
- `player_current_state_builder.py`: 約 340 行、14 個の依存
- `find_all()` で同スポット人数を数える処理が 2 箇所に重複（`_get_spot_context_for_player_impl` と `build_player_current_state`）
- WorldQueryService は demo / テストで直接インスタンス化されており、専用 wiring ファクトリは未整備

**成果物**:
- `PlayerLocationQueryService`: プレイヤー位置の取得（`_get_player_location_impl` を移行）
- `SpotContextQueryService`: スポット文脈の取得（`_get_spot_context_for_player_impl` を移行）
- `AvailableMovesQueryService`: 利用可能移動先の取得（`_get_available_moves_impl` を移行）
- `VisibleContextQueryService`: 視界・可視オブジェクトの取得（`build_visible_context` を移行。VisibleObjectReadModelBuilder, VisibleTileMapBuilder は既存を活用）
- `PlayerRuntimeContextBuilder`: インベントリ・会話・クエスト・ギルド・ショップ・取引・スキル等の runtime 情報を組み立て。既存の `PlayerSupplementalContextBuilder` が同様の責務を持つため、案 A（Supplemental をそのまま PlayerRuntimeContextBuilder として位置づけ）または案 B（Supplemental を保持する薄いファサードとして新規定義）を採用
- `WorldQueryService`: 上記を呼び出すだけの facade
- `create_world_query_service(...)`: 新規クエリサービス群の組み立てを一箇所に集約するファクトリ（`application/world/` または `world_query_wiring.py` に配置）

**PlayerAudienceQueryService の再利用**:
- 観測系で既存の `PlayerAudienceQueryService`（`players_at_spot(spot_id: SpotId) -> List[PlayerId]`）を world 系でも利用
- 利用箇所: `SpotContextQueryService`、`PlayerCurrentStateBuilder.build_player_current_state`
- 型変換: DTO では `current_player_ids: Set[int]` を用いるため、`{int(p.value) for p in player_audience.players_at_spot(spot_id)}` のように変換
- 配置検討: `IPlayerAudienceQueryPort` を `application/common/` または `application/world/contracts/` に置き、observation と world の両方から参照する形

**段階的進め方**（依存関係を考慮した推奨順）:
1. Phase B3a: `PlayerLocationQueryService` を切り出し（単独で切り出しやすい） - [x] **実施済み**（2026-03-15）
2. Phase B3a': `PlayerAudienceQueryService` の world 系利用。SpotContext と PlayerCurrentStateBuilder の「同スポット人数」取得を `players_at_spot` に統一 - [x] **実施済み**（2026-03-15）
3. Phase B3b: `SpotContextQueryService`, `AvailableMovesQueryService` を切り出し - [x] **実施済み**（2026-03-15）
4. Phase B3c: `VisibleContextQueryService` を切り出し（`PlayerCurrentStateBuilder.build_visible_context` を移行）
5. Phase B3d: `PlayerRuntimeContextBuilder` の整理（PlayerSupplementalContextBuilder との関係を確定）、`PlayerCurrentStateBuilder` を「現在状態」と runtime context のオーケストレータに縮小、WorldQueryService を facade 化
6. Phase B3e: `create_world_query_service` ファクトリを追加し、demo / テスト等の WorldQueryService インスタンス化箇所を置き換え

**変更箇所（インスタンス化）**:
- `scripts/demo_llm_prompt_inspection.py`, `scripts/demo_llm_multiturn_prompt_inspection.py`
- `tests/application/llm/test_prompt_builder.py`, `tests/application/world/services/test_world_query_service.py`, `tests/application/world/services/test_movement_service.py`, `tests/application/world/services/test_pursuit_command_service.py`

**テスト**: `test_world_query_service.py`（約 1058 行）, `test_player_current_state_builder.py`

**着手前の補修**:
- `world_query_service.py` 338 行付近で `List` を使用しているが import されていない。`from typing import List` を追加するか、`list[...]` に置き換えること

**参照パターン**: `movement_wiring.create_movement_application_service` による facade と子サービスの組み立て。`create_world_query_service` も同型で子クエリサービスを注入する形にする。

**呼び出し元（影響なし）**: PromptBuilder, PursuitCommandService, PursuitContinuationService, LlmAgentTurnRunner 等は `get_player_location` 等の public API のみ使用。facade 化後も API は維持するため、呼び出し元の変更は不要。

**リファクタリングの最終目的**:
- WorldQueryService を「子クエリサービスを呼び出すだけの薄い facade」に縮小し、依存爆発を抑える
- 責務ごとにクエリサービスを分離し、新規クエリ追加時の変更箇所を局所化する
- 同スポット人数取得などの重複ロジックを一箇所に集約する
- `create_world_query_service` ファクトリで組み立てを一箇所に寄せ、demo / テストのインスタンス化を統一する

**read-model の想定**:
- 本計画での「read-model」は、CQRS のイベントソーシング read model ではなく、「読み取り専用の視点（view）を組み立てる責務の分離」を指す
- `PlayerLocationDto`, `SpotInfoDto`, `VisibleContextDto`, `PlayerCurrentStateDto` 等は、LLM / UI 向けに最適化した読み取り用 DTO
- これらを組み立てるサービス（PlayerLocationQueryService, SpotContextQueryService 等）を「各 DTO に対応する read-model ビルダ」として責務別に切り出す
- 現状はリポジトリから直接取得しているが、将来イベント駆動の materialized view を挟む余地を残す設計意図はない（現時点では単純な責務分離）

**後方互換の扱い（Phase B3a / B3a' / B3b で導入した暫定措置）**:
- `WorldQueryService` の `player_location_query_service`, `player_audience_query`, `spot_context_query_service`, `available_moves_query_service` を Optional とし、未指定時は内部で生成している
- `PlayerCurrentStateBuilder` の `player_audience_query` を Optional とし、未指定時は `find_all()` によるフォールバックを残している
- **Phase B3e 完了時に削除する**: `create_world_query_service` ファクトリ導入後、demo / テストをすべてファクトリ経由に置き換え、上記パラメータを必須化し、内部生成とフォールバックを削除する

---

## 次のセッションへの引き継ぎ（2026-03-15 Phase B3b 完了時点）

**完了済み**:
- Phase B3a, B3a', B3b 実施済み
- `PlayerLocationQueryService`, `SpotContextQueryService`, `AvailableMovesQueryService` の切り出し完了
- WorldQueryService は上記 3 サービスを Optional 注入で委譲。未指定時は内部生成
- `test_spot_context_query_service.py`（11 件）, `test_available_moves_query_service.py`（13 件）を追加

**次に着手すべき Phase**:
- **Phase B3c**: `VisibleContextQueryService` の切り出し
  - `PlayerCurrentStateBuilder.build_visible_context` を移行
  - `build_visible_context` は `PlayerCurrentStateBuilder` が `VisibleObjectReadModelBuilder`, `VisibleTileMapBuilder` を組み合わせて呼び出している
  - 切り出し後は WorldQueryService の `_get_visible_context_impl` が `VisibleContextQueryService` を呼ぶ形に変更

**その後の順序**:
- Phase B3d: PlayerRuntimeContextBuilder の整理、PlayerCurrentStateBuilder のオーケストレータ化
- Phase B3e: `create_world_query_service` ファクトリ追加、demo / テストの置き換え
