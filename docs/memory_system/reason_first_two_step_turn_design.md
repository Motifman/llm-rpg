...
Phase A 入口で reason-first gate が成立した場合は reason-first を優先し、
`resolve_turn_reasoning_effort` は呼ばない。これにより「2 段階 + reasoning 付き 1 段階」の
二重発火を避け、LLM 呼び出しの意味を 1 turn につき 1 方式に保つ。
