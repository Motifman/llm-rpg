...
class TestSystemPromptIncludesHeading:
    ...
    def test_system_prompt_lists_heading_as_required_key(self) -> None:
        ...
        port = _StubSubjectivePort({'interpreted': 'x', 'recall_text': 'y', 'heading': 'h'})
...
class TestHeadingMergeFromLlmOutput:
    ...
    def test_llm_heading_is_written_to_episode(self) -> None:
        ...
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': '意味付け', 'recall_text': '想起テキスト', 'heading': '司書の手記を読んだ — 水の断片語'})
        svc = EpisodicChunkSubjectiveFieldsService(port)
    ...
    def test_long_heading_is_truncated(self) -> None:
        ...
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        long_text = 'あ' * 50
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'heading': long_text})
        svc = EpisodicChunkSubjectiveFieldsService(port)
        merged = svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
    ...
    def test_blank_heading_string_becomes_none(self) -> None:
        ...
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'heading': '   '})
        svc = EpisodicChunkSubjectiveFieldsService(port)
        merged = svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
        assert merged.heading is None
