...
MEMO_ADD_DESCRIPTION = (
    ...
    "memo_done は **memo を記録から外す** 操作です。**物理アクション (travel_to / interact / "
    "pickup_item / speech / use_item 等) で目標を達成した後**、もう参照不要になったら "
    "memo_done で閉じてください。memo_done を呼んだだけでは世界の状態は変わらない点に注意。"
    "**達成根拠が直近の出来事に残っているうちに完了させると snapshot の質が上がります。**"
)
...
MEMO_DONE_DESCRIPTION = (
    "指定 ID 群の memo を **記録から外す** ためのツールです。"
    "memo_done は『**メモを片付ける**』操作であり、**目標を達成したという宣言ではありません**。"
    "目標を達成するには、その目標に対応する物理ツール (travel_to / interact / pickup_item / "
    "speech / use_item など) を **別途呼ぶ必要があります**。memo_done を呼んだだけでは"
    "世界の状態は何も変わりません。"
    "\n\n"
    "memo_done を呼ぶべきタイミング:"
    "\n"
    "- **対応する物理アクションがすでに成功した** (例: 「枯れ葉を確保する」memo に対して "
    "  pickup_item / gather_dry_leaves が直前に success した) ので、もう参照不要になった\n"
    "- 状況が変わって memo の前提が消えた (例: 「カイトと合流する」memo だがカイトが別目標を選んだ)\n"
    "- 重複や誤った memo を撤回したい"
    "\n\n"
    "memo を読んで「あ、これは前にやろうとしたことだ」と想起しただけでは閉じないでください。"
    "想起 → memo_done を反射的に繰り返すと、**実行を伴わない形式的な処理ループ** に陥り、"
    "目標が世界に反映されないまま記憶だけが流れていきます。"
    "\n\n"
    "1 件だけ完了する場合も memo_ids=['<id>'] のように単一要素配列で渡してください。"
    "**同じ目的を構成する複数の memo (= 同じ物理アクションで一斉に達成したもの) を一気に閉じる**"
    "と context が軽くなります。"
    "\n\n"
