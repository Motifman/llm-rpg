"""
ItemDroppedFromInventoryEvent を受けて、意図的に捨てられたアイテムをプレイヤー位置にドロップ（GROUND_ITEM）する同期ハンドラ。
...
class ItemDroppedFromInventoryDropHandler(EventHandler[ItemDroppedFromInventoryEvent]):
    """意図的に捨てられたアイテムをその場にドロップするハンドラ"""
    ...
    def handle(self, event: ItemDroppedFromInventoryEvent) -> None:
        try:
            self._handle_impl(event)
        except (DomainException, SystemErrorException):
            raise
        except Exception as e:
            self._logger.exception("Unexpected error in ItemDroppedFromInventoryDropHandler: %s", e)
            raise SystemErrorException(
                f"Item dropped from inventory handling failed: {e}",
                original_exception=e,
            ) from e

    def _handle_impl(self, event: ItemDroppedFromInventoryEvent) -> None:
        ...
        if not player_status or not player_status.current_spot_id or player_status.current_coordinate is None:
            self._logger.debug(
                "Player status or position not found for player_id=%s, cannot drop item",
                event.aggregate_id,
            )
            return
        ...
        if not physical_map:
            self._logger.debug(
                "Physical map not found for spot_id=%s, cannot drop item",
        ...
        component = GroundItemComponent(item_instance_id=event.item_instance_id)
        ...
        self._logger.debug(
            "Dropped item %s at spot %s coord %s for player %s",
