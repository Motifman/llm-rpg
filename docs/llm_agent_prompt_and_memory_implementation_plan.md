...
### 5.4 目的地設定とティック移動を分けない方針

**結論**: **set_destination**（目的地・経路の設定）と **tick_movement**（経路に沿った 1 ステップ実行）を、LLM 用の**別ツールとして分ける必要はない**。プレイヤー体験としては「どこかへ行く」は 1 つの行動として扱い、移動は 1 本のツール（例: 目的地を指定して移動を開始する）にまとめる方針とする。

- **現状の実装**: 内部では SetDestinationCommand（経路設定）とゲームループ側のティック進行が分かれているが、LLM に露出するツールは **move_set_destination** のみ。LLM は「目的地を指定する」だけでよく、実際の 1 マスずつの進行はゲームループが行うため、tick_movement を別ツールとして LLM に渡す必要はない。
- **分けない理由**: 「目的地を決める」と「1 マス進む」を別ツールにすると、LLM が毎ターン tick_movement を選ぶ必要が出て不自然になる。1 ターン 1 ツール呼び出しの前提では、移動は「目的地を指定して移動を開始する」1 アクションにまとめるのが自然である。
