...
U3b (固着パス): ``belief_evidence_buffer_store`` を注入すると
「FAMILIARITY 転用モード」になる。semantic_learning_consolidation_design.md
「既存機構の役割整理」節が指示するとおり、store への直接書き込みをやめ、
検出したクラスタを ``BeliefEvidenceSourceKind.FAMILIARITY`` の
``BeliefEvidence`` として evidence buffer に emit し、belief journal への
実際の書き込みは ``BeliefConsolidationCoordinator`` (固着パス) に委ねる。
このモードでは ``recall_count>=3`` ゲート (``MIN_RECALL_COUNT``) も外れる
(学習のゲートではなく、本来の役割であるエピソード想起の spreading /
優先度に純化する設計判断)。未注入 (default) なら従来どおり store 直書き +
ゲート付きの挙動を維持する (= flag OFF での並存期間の後方互換)。
"""
...
def _build_strong_adjacency(
    ...
    """実効強度 >= 閾値の無向辺を隣接リスト化する (being_id keyed)。

    Phase 3 Step 3c-3: legacy player_id 経路は撤去済。
    """
    adj: Dict[str, Set[str]] = {}
    for ln in link_store.list_all_links_for_being(being_id):
...
def _expand_frontier_nodes(
    ...
    for _ in range(max(0, max_hops)):
        nxt: Set[str] = set()
        for n in frontier:
            for ln in link_store.list_all_incident_links_by_being(
                being_id, n, now=now
            ):
...
def _build_strong_adjacency_for_nodes(
    ...
    """nodes 内の頂点のみを対象に、強リンクで誘導する無向グラフの隣接リスト
    (being_id keyed)。"""
    adj: Dict[str, Set[str]] = {}
    for n in nodes:
        for ln in link_store.list_all_incident_links_by_being(
...
class EpisodicSemanticClusterPromotionService:
    ...
    def on_after_tool_turn(self, player_id: int, *, now: datetime | None = None) -> None:
        ...
        if not adj:
            return
        # U3b: 注入されていれば FAMILIARITY 転用モード。store 直書きをやめ
        # evidence buffer に emit するので、recall_count>=3 ゲートも外れる
        # (「学習のゲートではなくエピソード想起の spreading/優先度に純化する」
        # という設計判断)。
        familiarity_mode = self.belief_evidence_buffer_store is not None
        for comp in _connected_components(adj):
            ...
            for eid in comp:
                ...
                if ep is None:
                    cluster_ok = False
                    break
                if not familiarity_mode and ep.recall_count < MIN_RECALL_COUNT:
                    cluster_ok = False
                    break
                eps.append(ep)
    ...
    def _emit_familiarity_evidence(
        ...
        """検出したクラスタを FAMILIARITY ``BeliefEvidence`` として buffer に積む。

        text はクラスタ代表 (決定論 gist の concat)、cue_signature は
        代表 episode (最古の 1 件) の構造化フィールドから決定論生成する
        (= 新しい抽出ロジックを発明しない、U2 の方針を踏襲)。belief journal
        への実際の反映は固着パス (``BeliefConsolidationCoordinator``) に
        委ねる。
        """
        if self.belief_evidence_buffer_store is None:
            return
        ordered = sorted(cluster_episodes, key=lambda e: e.occurred_at)
        representative = ordered[0]
        evidence = BeliefEvidence(
            evidence_id=f"belief-evidence-{uuid4().hex}",
            source_kind=BeliefEvidenceSourceKind.FAMILIARITY,
            episode_ids=tuple(sorted(comp)),
            cue_signature=build_belief_evidence_cue_signature(representative),
            text=_deterministic_gist(ordered),
            salience=BELIEF_EVIDENCE_SALIENCE_LOW,
            occurred_at=now,
        )
        self.belief_evidence_buffer_store.append_by_being(being_id, evidence)
