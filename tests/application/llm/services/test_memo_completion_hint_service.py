...
class TestMemoCompletionHintDetect:
    ...
    def test_未完了_memo_が_無ければ_None_を返す(self, player_id: PlayerId) -> None:
        """memo_store が空なら hint なし (None)。"""
        store = InMemoryMemoStore()
        service = MemoCompletionHintService(memo_store=store)
        assert service.detect(player_id, "act", "res") is None

    def test_類似度が閾値未満なら_None_を返す(self, player_id: PlayerId) -> None:
        """全 memo が閾値未満なら hint なし。"""
        store = InMemoryMemoStore()
        store.add(player_id, "金庫室で扉固定スイッチを押す")
        service = MemoCompletionHintService(memo_store=store)
        # 全く無関係な行動
    ...
    def test_複数_memo_の中から最も類似度の高いものを選ぶ(
        ...
        store = InMemoryMemoStore()
        store.add(player_id, "リンと合流する")
        target_id = store.add(player_id, "金庫室で扉固定スイッチを押す")
        service = MemoCompletionHintService(
            memo_store=store, similarity_threshold=0.3
        )
        result = service.detect(
