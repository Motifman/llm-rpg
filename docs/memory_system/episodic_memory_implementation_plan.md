...
- いつチャンクを閉じエピソード生成へ進むかの純粋ルールは `application.llm.chunk_boundary`（`decide_chunk_boundary`, `summarize_observation_boundary_hints`）。区間に行動が足りるかのゲートは `chunk_encoding.chunk_encoding_episode_generation_allowed` と同値（第 1 版は `ActionResultEntry` が 1 件以上）。
