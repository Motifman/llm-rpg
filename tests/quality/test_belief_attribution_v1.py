...
def _dump_belief_attribution(
    ...
    # 自動採番 id は畳んでから書く。生の uuid4 のままだと、再生成する
    # たびに差分が出て「何が変わったのか」が読めなくなる。
    path.write_text(fold_autogenerated_ids("".join(parts)), encoding="utf-8")
    return path
...
class TestBeliefAttributionV1:
    ...
    def test_context_belief_cue_zero_confirmation(
        ...
        processed = coordinator.flush_player(PlayerId(1), being_id)
