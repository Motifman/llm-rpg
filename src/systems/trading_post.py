...
from ..models.trade import TradeOffer, TradeStatus, TradeType
...
class TradingPost:
    ...
    def accept_trade(self, trade_id: str, buyer_id: str) -> TradeOffer:
        ...
        if not trade.can_be_accepted_by(buyer_id):
            ...
            elif trade.target_agent_id and trade.target_agent_id != buyer_id:
                raise ValueError("この取引は他のエージェント向けです")
            else:
        ...
        completed_trade = TradeOffer(
            ...
            target_agent_id=trade.target_agent_id,
    ...
    def get_agent_trades(self, agent_id: str, include_history: bool = False) -> List[TradeOffer]:
        """指定エージェントの取引一覧を取得"""
        agent_trades = []
    ...
    def get_available_trades_for_agent(self, agent_id: str) -> List[TradeOffer]:
        """指定エージェントが受託可能な取引一覧を取得"""
        available_trades = []
        
        for trade in self.active_trades.values():
            if trade.can_be_accepted_by(agent_id):
