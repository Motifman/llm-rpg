...
- [x] 失敗時（タイムアウト・5xx・不正 JSON・スキーマ不一致）の **リトライ方針**がコードで明示されている（`EpisodeEncodingProcessor.max_retries` が **同一 `process_pending` 呼び出し内**の encoder 再試行。それでも失敗した場合は **`max_queue_retries` と指数バックオフ**で `pending_encoding` に戻し、将来の `process_pending` で再処理。恒久的欠損（trace 解決 `ValueError`）は `encoding_failed` のまま）。
...
- **リトライと `pending`**: `EpisodeEncodingProcessor` は (1) `max_retries` で **同一 `process_pending` 呼び出し内**の encoder 呼び出しを繰り返す。(2) それでも失敗した場合は **`encoding_retry_count` と `last_encoding_failure_at` を更新**し `pending_encoding` のまま残し、指数バックオフ経過後にのみ再処理する。`max_queue_retries` を超えたら `encoding_failed`。(3) **trace 解決の `ValueError`**（欠損 trace 等）は恒久的とみなし **その場で `encoding_failed`**（キュー再試行なし）。
...
- `encoding_failed` のバッチ跨ぎ再試行は **実装済み**（`EpisodeEncodingProcessor` のキュー側リトライ。詳細は Phase 3 実装メモ）。
...
| `encoding_failed` | **バッチ跨ぎ再試行**: `EpisodeEncodingProcessor` のキュー側リトライ（指数バックオフ・上限付き）を実装済み。恒久的欠損は `encoding_failed` のまま。 |
...
- **`encoding_failed`**: キュー側の自動再試行を実装済み（指数バックオフ・`max_queue_retries`）。恒久的欠損は `encoding_failed` のまま。
