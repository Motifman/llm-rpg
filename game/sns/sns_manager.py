...
class SnsManager:
    ...
    def _process_mentions_in_content(self, content: str, user_id: str, post_id: str, reply_id: Optional[str] = None) -> List[str]:
        ...
        # 重複を除去
        unique_mentions = list(set(mentioned_usernames))
        
        processed_mentions = []
        for username in unique_mentions:
            # ユーザーIDとして直接検索（テストでは@user2のようにuser_idでメンション）
            mentioned_user_id = username if self.user_exists(username) else None
