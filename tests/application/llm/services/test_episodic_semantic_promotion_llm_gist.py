...
from ai_rpg_world.application.llm.services.semantic_gist_service import SemanticGistResult

def _make_episode(*, episode_id: str, player_id: int=1, occurred_at_minute: int=0, recall_text: str='ある記憶', recall_count: int=5, interpreted: str | None=None) -> SubjectiveEpisode:
...
def _build_cluster(*, gist_service=None, persona_resolver=None):
    ...
    for (i, eid) in enumerate(['x', 'y', 'z']):
        ep = _make_episode(episode_id=eid, occurred_at_minute=i, recall_text=f'記憶{i}', recall_count=5, interpreted=f'主観文{i}')
    ...
    svc = EpisodicSemanticClusterPromotionService(episode_store=episode_store, link_store=link_store, semantic_store=setup.semantic_store, promotion_frontier=None, gist_service=gist_service, persona_resolver=persona_resolver)
...
class TestEpisodicSemanticClusterPromotionLlmGist:
    ...
    def test_gist_service_success_llm_text_importance_tags_included(self) -> None:
        ...
        stub = _StubGistService(result=SemanticGistResult(gist_text='タカシは信頼できる', importance_score=8, tags=('タカシ', '信頼')))
        (svc, sem_store, being_id) = _build_cluster(gist_service=stub, persona_resolver=lambda pid: ('ハル', '慎重で寡黙'))
        svc.on_after_tool_turn(1, being_id)
        entries = sem_store.list_entries(1)
        assert len(entries) == 1
        entry = entries[0]
    ...
    def test_gist_service_gist_fallback_raises_exception(self, caplog: pytest.LogCaptureFixture) -> None:
        """LLM 失敗時 (API 例外 / ValueError) は決定論 gist に縮退し warning を出す。"""
        stub = _StubGistService(exc=LlmApiCallException('simulated', error_code='LLM_API_CALL_FAILED'))
        (svc, sem_store, being_id) = _build_cluster(gist_service=stub)
        with caplog.at_level(logging.WARNING, logger='ai_rpg_world.application.llm.services.episodic_semantic_cluster_promotion'):
            svc.on_after_tool_turn(1, being_id)
        entries = sem_store.list_entries(1)
        assert len(entries) == 1
        assert '主観文0' in entries[0].text
        assert any(('falling back to deterministic gist' in rec.message for rec in caplog.records))

    def test_persona_resolver_llm_completes(self) -> None:
        """persona_resolver の例外は warning を出して空文字 persona で続行。"""
        stub = _StubGistService(result=SemanticGistResult(gist_text='ok', importance_score=5, tags=()))

        def broken_resolver(pid: int):
            raise RuntimeError('oops')
        (svc, sem_store, being_id) = _build_cluster(gist_service=stub, persona_resolver=broken_resolver)
        svc.on_after_tool_turn(1, being_id)
        entries = sem_store.list_entries(1)
        assert len(entries) == 1
    ...
    def test_persona_resolver_unspecified_default_player_x_works(self) -> None:
        ...
        stub = _StubGistService(result=SemanticGistResult(gist_text='ok', importance_score=5, tags=()))
        (svc, sem_store, being_id) = _build_cluster(gist_service=stub, persona_resolver=None)
        svc.on_after_tool_turn(1, being_id)
        entries = sem_store.list_entries(1)
        assert len(entries) == 1
