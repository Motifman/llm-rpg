...
from ai_rpg_world.application.llm.services.executors.episodic_memory_recall_tool_executor import DEFAULT_MAX_RESULTS, EMPTY_RESULT_MESSAGE, EpisodicMemoryRecallToolExecutor
...
def _episode(*, episode_id: str, occurred_at: datetime, spot_id: int=1, what: str='閲覧室で覚書を読んだ', recall_text: str='QUALITY_RECALL: 閲覧室で覚書を読んだ', cues: Optional[tuple]=None) -> SubjectiveEpisode:
    if cues is None:
        cues = (EpisodicCue(axis='place_spot', value=str(spot_id), source=EpisodicCueSource.RUNTIME_CONTEXT),)
    return SubjectiveEpisode(episode_id=episode_id, player_id=_PLAYER_ID_INT, being_id=BeingId(f'being_w1_p{_PLAYER_ID_INT}'), occurred_at=occurred_at, game_time_label=None, source=EpisodeSource(event_ids=(f'evt-{episode_id}',)), location=EpisodeLocation(spot_id=spot_id), action=EpisodeAction(tool_name='travel_to'), who=('player_lin',), what=what, why=None, observed=what, expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=cues, recall_text=recall_text)
...
def _build_executor(store: InMemorySubjectiveEpisodeStore, *, with_matcher: bool=True) -> EpisodicMemoryRecallToolExecutor:
    return EpisodicMemoryRecallToolExecutor(episode_store=store, noun_matcher=_build_matcher() if with_matcher else None, time_provider=lambda : _NOW)
...
class TestEmptyResult:
    ...
    def test_about_no_match_no_time_match_returns_empty(self) -> None:
        ...
        store.put_by_being(being, _episode(episode_id='too-old', occurred_at=_NOW - timedelta(days=10)))
        executor = _build_executor(store)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
...
class TestAboutCueMatching:
    ...
    def test_about_with_known_spot_name_recalls_matching_episode(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='reading-room', occurred_at=_NOW - timedelta(hours=18), spot_id=1, recall_text='昨日の昼、閲覧室で覚書を読んだ'))
        store.put_by_being(being, _episode(episode_id='shelf-a', occurred_at=_NOW - timedelta(hours=18), spot_id=2, recall_text='書架Aで断片語を見つけた'))
        executor = _build_executor(store)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
    ...
    def test_no_noun_in_about_falls_back_to_temporal(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='yesterday-ep', occurred_at=_NOW - timedelta(hours=18), recall_text='昨日の出来事'))
        executor = _build_executor(store)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
        result = handler(acting, {'about': '俺昨日何したっけ?', 'time_range': 'yesterday'})
        assert result.success is True
...
class TestTimeRange:
    ...
    def test_today_excludes_yesterday(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='recent', occurred_at=_NOW - timedelta(hours=2), recall_text='今日の出来事'))
        store.put_by_being(being, _episode(episode_id='old', occurred_at=_NOW - timedelta(hours=40), recall_text='昨日のさらに前'))
        executor = _build_executor(store)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
    ...
    def test_any_does_not_filter(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='ancient', occurred_at=_NOW - timedelta(days=30), recall_text='ずっと昔の出来事'))
        executor = _build_executor(store)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
    ...
    def test_unknown_time_range_treated_as_no_filter(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='recent', occurred_at=_NOW - timedelta(hours=2), recall_text='出来事'))
        executor = _build_executor(store)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
...
class TestNounMatcherUnavailable:
    ...
    def test_no_matcher_falls_back_to_temporal_recent(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='recent', occurred_at=_NOW - timedelta(hours=2), recall_text='直近のこと'))
...
class TestResultLimit:
    ...
    def test_caps_at_default_max_results(self) -> None:
        ...
        for i in range(DEFAULT_MAX_RESULTS + 5):
            store.put_by_being(being, _episode(episode_id=f'ep-{i}', occurred_at=_NOW - timedelta(minutes=i), recall_text=f'recall-{i}'))
        executor = _build_executor(store, with_matcher=False)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
...
class TestNoIdLeakInOutput:
    ...
    def test_message_contains_no_internal_ids(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='ep', occurred_at=_NOW - timedelta(hours=2), spot_id=42, recall_text='閲覧室で覚書を読んだ'))
        executor = _build_executor(store, with_matcher=False)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
    ...
    def test_episode_id_not_leaked_when_episode_id_is_numeric(self) -> None:
        ...
        (acting, being) = _make_acting()
        being = acting.being_id
        store.put_by_being(being, _episode(episode_id='12345-numeric-uuid', occurred_at=_NOW - timedelta(hours=2), spot_id=42, recall_text='昨日の出来事の説明'))
        executor = _build_executor(store, with_matcher=False)
        handler = executor.get_handlers()[TOOL_NAME_MEMORY_RECALL_EPISODES]
