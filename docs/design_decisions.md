...
## 34. `is_outdoor` 未宣言の屋内扱いは validator で見える化する
...
屋内意図は読み取れないため、v4 では `is_outdoor: true` を明示する。この変更により、
次の本命 run では拠点から「切り立った山影」が見える。run 003 との比較では、この
目標 cue の露出条件が変わったことを明記する。
