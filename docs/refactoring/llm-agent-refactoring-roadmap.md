...
- [ ] **follow-up 継続中**（facade の slim 化と残存 helper 整理）
...
**現状**:
- `WorldSimulationApplicationService` は 6 stage へ委譲する facade まで整理済み
- `HungerMigrationPolicy`、`MonsterBehaviorCoordinator`、`MonsterLifecycleSurvivalCoordinator` により monster 系業務ルールは主要部分を分離済み
- 契約別 integration test と stage/coordinator test による regression harness も追加済み
- 一方で facade 本体は 1140 行あり、未使用 helper と collaborator 組み立て責務が残っている
