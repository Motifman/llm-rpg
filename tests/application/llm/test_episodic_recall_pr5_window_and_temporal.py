...
import pytest
from ai_rpg_world.application.llm.services.episodic_passive_recall_retrieval import PASSIVE_RECALL_AXIS_TEMPORAL, EpisodicPassiveRecallRetrievalService
...
from ai_rpg_world.domain.memory.episodic.value_object.episodic_cue import EpisodicCue, EpisodicCueSource
from ai_rpg_world.domain.memory.episodic.value_object.subjective_episode import SubjectiveEpisode
from ai_rpg_world.domain.player.value_object.player_id import PlayerId
_being = BeingId('being_w1_p1')

def _episode(*, episode_id: str, occurred_at: datetime, cues: tuple[EpisodicCue, ...]=()) -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=episode_id, player_id=1, being_id=BeingId('being_w1_p1'), occurred_at=occurred_at, game_time_label=None, source=EpisodeSource(event_ids=('evt',)), location=EpisodeLocation(spot_id=None), action=EpisodeAction(tool_name='test_tool'), who=('p',), what=episode_id, why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=cues, recall_text=f'recall:{episode_id}')
...
class TestR1WindowOuterFilter:
    ...
    def test_missing_min_occurred_at_allows_all_episodes(self) -> None:
        ...
        result = svc.retrieve(being_id=_being, situation_cues=(cue,), limit_per_axis=10, max_candidates=10)
    ...
    def test_min_occurred_episode_excluded(self) -> None:
        ...
        result = svc.retrieve(being_id=_being, situation_cues=(cue,), limit_per_axis=10, max_candidates=10, min_occurred_at=border)
        ids = {c.episode.episode_id for c in result.candidates}
        assert ids == {'old'}
    ...
    def test_boundary_episode_excluded_strictly_older(self) -> None:
        ...
        svc = EpisodicPassiveRecallRetrievalService(store)
        result = svc.retrieve(being_id=_being, situation_cues=(cue,), limit_per_axis=10, max_candidates=10, min_occurred_at=ts)
    ...
    def test_naive_aware_utc(self) -> None:
        ...
        store = InMemorySubjectiveEpisodeStore()
        cue = EpisodicCue(axis='action', value='open', source=EpisodicCueSource.TOOL)
        store.put_by_being(_being, _episode(episode_id='aware-old', occurred_at=datetime(2026, 5, 1, tzinfo=timezone.utc), cues=(cue,)))
        svc = EpisodicPassiveRecallRetrievalService(store)
        border_naive = datetime(2026, 6, 1)
        result = svc.retrieve(being_id=_being, situation_cues=(cue,), limit_per_axis=10, max_candidates=10, min_occurred_at=border_naive)
        ids = {c.episode.episode_id for c in result.candidates}
...
class TestR2TemporalAxisFallback:
    ...
    def test_cue_idle_turn_temporal_episode(self) -> None:
        ...
        svc = EpisodicPassiveRecallRetrievalService(store)
        result = svc.retrieve(being_id=_being, situation_cues=(), limit_per_axis=10, max_candidates=10)
        ids = [c.episode.episode_id for c in result.candidates]
        assert ids == ['t2', 't1']
...
class TestR2TemporalFallbackPinning:
    ...
    def test_empty_observation_prose_and_runtime_context_return_empty_cues(self) -> None:
        ...
        cues = build_situation_episodic_cues(runtime_context=ToolRuntimeContextDto.empty(), observation_structured=None, latest_action=None, observation_prose=None, noun_matcher=None)
        assert tuple(cues) == ()
...
class TestSlidingWindowOldestEntryDatetime:
    ...
    def test_one_append_window_entry_occurred(self) -> None:
        ...
        mem = DefaultSlidingWindowMemory()
        ts = datetime(2026, 6, 1, tzinfo=timezone.utc)
        mem.append(PlayerId(1), ObservationEntry(occurred_at=ts, output=ObservationOutput(prose='x', structured={}, observation_category='self_only', schedules_turn=False, breaks_movement=False), game_time_label=None))
...
class TestPromptBuilderDefenseAgainstBadOldestType:
    ...
    def test_emits_warning_for_none(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        class _BrokenSlidingWindow(IShortTermMemory):
            ...
            def append(self, player_id, entry):
                pass

            def append_all(self, player_id, entries):
                return []

            def get_recent(self, player_id, limit):
                return []

            def complete_turn(self, player_id):
                pass

            def get_oldest_entry_datetime(self, player_id):
        ...
        if raw_oldest is not None and (not isinstance(raw_oldest, datetime)):
            with caplog.at_level(logging.WARNING):
                pb_mod._module_logger.warning('IShortTermMemory.get_oldest_entry_datetime returned unexpected type %s for player_id=%s; recall の時間下限フィルタ を off にして fallback します。', type(raw_oldest).__name__, 1)
            min_recall_dt = None
        else:
