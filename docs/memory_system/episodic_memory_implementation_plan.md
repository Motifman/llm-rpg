...
**チャンク境界の判定（第 1 版・実装）**: 協調オブジェクトはバッファを持たず、`ChunkEncodingInput` とオプションの `ChunkBoundaryObservationHints`（例: drain 件数）を `application.llm.services.episode_chunk_boundary.decide_episode_chunk_boundary` に渡す。`EpisodeChunkBoundaryPolicy` でしきい値を固定でき、観測カテゴリ・structured キー集合の変化・件数上限・drain ヒントで CLOSE を補強する。`chunk_encoding_episode_generation_allowed` と整合し、行動 0 件区間では必ず DEFER。
