...
2. Phase B1c: `MapInteractionPolicy` と `ChestInteractionPolicy` を切り出し、`interact_with` を整理（**B1b より先に着手推奨**）
   - [x] **実施済み**（2026-03-15）
3. Phase B1b: `HarvestSessionPolicy` を切り出し、採取開始の事前検証を移行
   - [x] **実施済み**（2026-03-15: HarvestSessionPolicy.validate_can_start_harvest）
...
**段階的進め方**:
1. Phase B2a: `ArrivalPolicy` と `PassableAdjacentFinder` をリポジトリ非依存のヘルパーとして切り出し - [x] **実施済み**（2026-03-15）
2. Phase B2b: `MoveResultAssembler` で DTO 組み立てを分離 - [x] **実施済み**（2026-03-15）
3. Phase B2c: `SetDestinationService` と `MovementStepExecutor` を分離し、facade を薄くする
   - [x] **Phase B2c-1 実施済み**（2026-03-15: SetDestinationService の切り出し、MovementApplicationService が SetDestinationService を注入して利用）
   - [x] **Phase B2c-2a 実施済み**（2026-03-15: MovementStepExecutor の切り出し、MovementApplicationService が MovementStepExecutor を注入して委譲）
   - [x] **Phase B2c-2b 実施済み**（2026-03-15: movement_wiring.create_movement_application_service による明示的注入）
...
**段階的進め方**:
1. Phase B3a: `PlayerLocationQueryService` を切り出し - [x] **実施済み**
2. Phase B3b: `SpotContextQueryService`, `AvailableMovesQueryService` を切り出し - [x] **実施済み**
3. Phase B3c: `VisibleContextQueryService` を整理（既存 builder の活用） - [x] **実施済み**（2026-03-15）
4. Phase B3d: `PlayerRuntimeContextBuilder` を PlayerCurrentStateBuilder から分離し、WorldQueryService を facade 化 - [x] **実施済み**（2026-03-15）
5. Phase B3e: `create_world_query_service` ファクトリを追加 - [x] **実施済み**（2026-03-15）

**テスト**: `test_world_query_service.py`, `test_player_current_state_builder.py`, `test_visible_context_query_service.py`, `test_player_runtime_context_builder.py`, `test_world_query_wiring.py`
...
**段階的進め方**:
1. Phase C1a: `MonsterBehaviorStateMachine` を切り出し、遷移規則を集約 - [x] **実施済み**（2026-03-15）
2. Phase C1b: `MonsterLifecycleState`, `MonsterPursuitState` を値オブジェクトとして切り出し - [x] **実施済み**
3. Phase C1c: aggregate を調停役に縮小 - [x] **実施済み**（2026-03-15）

**Phase C1c 実施内容**（2026-03-15 完了）:
- `FeedMemory`: 餌場記憶の値オブジェクト化（LRU 最大3件、`remember`/`cleared`/`empty`）
- `MonsterBehaviorState`: 行動状態（state, target_id, last_known_position, initial_position 等）の値オブジェクト化
- `MonsterAggregate`: `_behavior_state` を `MonsterBehaviorState`、`_feed_memory` を `FeedMemory` に委譲し、調停役として縮小
- テスト: `test_monster_aggregate.py`、`test_world_simulation_service.py` の `_behavior_state` 直接代入箇所を `MonsterBehaviorState.from_legacy()` に修正

**Phase C1c 設計上の注意**:
- **コンストラクタのシンプル化（2026-03-15 完了）**: `MonsterAggregate.__init__` は `MonsterBehaviorState` と `FeedMemory` を直接受け取る形に変更済み。`reconstitute` クラスメソッドでスポーン済み状態の再構成をサポート。`MonsterBehaviorState.from_parts()` は個別フィールドから構築するファクトリ（永続化層・テスト用）。

**Phase C1a 実施内容**:
- `domain/monster/service/monster_behavior_state_machine.py` を新規作成
- `compute_attacked_transition()`: 被弾時の遷移（PATROL_ONLY/FLEE_ONLY/AMBUSH/NORMAL）を集約
- `apply_transition()`: StateTransitionResult の適用ロジック（ENRAGE/FLEE/spot_target/lose_target）を集約
- `should_return_to_territory()`: テリトリ超過判定を集約
- `MonsterAggregate` は StateMachine を委譲先とし、`record_attacked_by`、`apply_behavior_transition`、`apply_territory_return_if_needed` を縮小
- `_emit_behavior_event()`: EventSpec に基づくイベント発行ヘルパーを追加
- テスト: `test_monster_behavior_state_machine.py` を新規作成（32 件、正常・境界・例外を網羅）

**テスト**: `test_monster_aggregate.py`（約 1871 行）、`test_monster_behavior_state_machine.py`（32 件）
...
**段階的進め方**:
1. Phase C2a: `PlayerNavigationState` を値オブジェクトとして切り出し - [x] **実施済み**（2026-03-15）
2. Phase C2b: `PlayerPursuitState` を値オブジェクトとして切り出し - [x] **実施済み**（2026-03-16）
3. Phase C2c: aggregate を調停役に縮小 - [x] **実施済み**（2026-03-16）

**Phase C2c 実施内容**:
- テストにおける `_navigation_state` の直接 mutation を削除し、`clear_path()` またはコンストラクタの `navigation_state` に統一
- カプセル化の徹底: 内部状態の直接参照を排除。各テストの `_create_sample_status` / `_make_status` に `navigation_state` 引数を追加
- `pursuit_state` 型の統一: `Union[PlayerPursuitState, PursuitState]` を廃止し、`Optional[PlayerPursuitState]` のみに（影響範囲が狭かったため実施）
- `reconstitute` は不要（InMemory 永続化・コンストラクタ + `from_parts` で十分）
- `PlayerResources` は本フェーズでは導入しない

**Phase C2a 実施内容**（Monster パターンに準拠・2026-03-15 完了）:
- `PlayerNavigationState`: 位置・経路・目的地の不変値オブジェクト
  - `empty()`: 初期状態作成
  - `from_parts()`: 個別フィールドから構築（永続化層・テスト用）
  - `with_destination_set()`, `cleared()`, `advance_step()`, `with_location_updated()`
- `PlayerStatusAggregate`: コンストラクタで `navigation_state: Optional[PlayerNavigationState] = None` を受け取る。None のとき `PlayerNavigationState.empty()` を使用。Monster と同様に破壊的変更を許容し、呼び出し元は `PlayerNavigationState.from_parts()` で構築して渡す
- `planned_path` は `Tuple[Coordinate, ...]` で不変に保持。`advance_step()` は `(Optional[Coordinate], PlayerNavigationState)` を返す

**Phase C2b 実施内容**（Monster パターンに準拠・2026-03-16 完了）:
- `PlayerPursuitState`: 追跡状態の不変値オブジェクト
  - `empty()`: 初期状態（追跡なし）作成
  - `from_parts(pursuit: Optional[PursuitState])`: 永続化層・テスト用
  - `cleared()`: 追跡解除
  - `with_started(actor_id, target_id, target_snapshot, last_known)`: 追跡開始
  - `with_updated(target_snapshot, last_known)`: 追跡更新（変更なければ self を返す）
- `PlayerStatusAggregate`: コンストラクタで `pursuit_state: Optional[PlayerPursuitState] = None` を受け取る。None のとき `PlayerPursuitState.empty()` を使用
- `pursuit_state` プロパティは `Optional[PursuitState]` を返す（外部 API 互換維持）
- `_coerce_last_known` は aggregate 内に残す（調整責務）

**テスト**: `test_player_status_aggregate.py`、`test_player_navigation_state.py`、`test_player_pursuit_state.py`（新規、17 件、正常・境界・例外を網羅）
...
- [llm-wiring-refactoring-plan.md](./llm-wiring-refactoring-plan.md) によると Phase 1–6 は**すべて完了**済み。
- `_build_memory_stack`, `_build_reflection_stack`, `_build_tool_stack`, `_build_observation_stack`, `_build_prompt_stack` が既に導入済み。
- ToolCommandMapper の handler map、ToolArgumentResolver のツール別分割、ToolDefinitions のカテゴリ分割も実施済み。

### 5.2 結論: 追加の大規模リファクタリングは不要

現状の LLM wiring は composition root として十分に責務分離されており、**追加で必須のリファクタリングは不要**と判断する。

- ツール追加・メモリ切替・観測スタック変更は、既存の `_build_*` 経由で変更箇所が明確。
- 依存が多く `create_llm_agent_wiring` の引数が 50 個超であるが、必須パラメータとオプショナルが整理されており、実務上のボトルネックにはなっていない。
- 将来の改善候補（優先度低）:
  - `LlmWiringConfig` 等の設定オブジェクトで引数をまとめる。
  - `_build_tool_handler_map` の各 service 引数に Protocol を導入し `Optional[Any]` を削減。

**Track C（Domain 系）や Track D（細かい修正）を優先し、LLM wiring の追加整理は「あればよい」程度の位置づけとする。**

---

## 6. Track D: 細かい部分の整理（Phase D）

完了済みリファクタリング箇所のコードレビューで判明した残課題を、Phase D として整理する。  
規模は小さいが、型安全性・設計一貫性・テスタビリティの観点で改善の余地がある。

### 6.1 Phase D1: Optional[Any] と型の具体化

- [x] **Phase D1-1 実施済み**（2026-03-16）: `world_query_service.py` と `world_query_wiring.py` の型具体化
- [x] **Phase D1-2 実施済み**（2026-03-16）: `world_simulation_movement_stage_service.py` と `world_simulation_harvest_stage_service.py` の getter 型具体化
- [x] **Phase D1-3 実施済み**（2026-03-16）: `name_resolver.py` の型具体化

| 対象 | 内容 | 優先度 |
|------|------|--------|
| `world_query_service.py` L100 | `world_time_config_service: Optional[Any]` → `Optional[WorldTimeConfigService]` | 中 ✅ |
| `world_query_wiring.py` | 各 `Optional[Any]` を具体的な型（`QuestRepository`, `GuildRepository` 等）に置換 | 低 ✅ |
| `llm/wiring/__init__.py` | `_build_tool_handler_map` の各 service 引数に Protocol を導入（将来的に） | 低 |

### 6.2 Phase D2: WorldSimulationService の構築後 mutation の解消

**問題**: `WorldSimulationApplicationService.__init__` で `CollaboratorFactory.build()` 後、各 stage に getter を後から代入している。

```python
# world_simulation_service.py 169-184行付近
self._environment_stage._weather_config_service_getter = lambda: self._weather_config_service
...
**改善案**: `WorldSimulationCollaboratorFactory` の build 時に、movement/harvest 等の依存をコンストラクタ経由で渡せる形にし、post-construction の getter 代入をやめる。または、stage クラスがコンストラクタで getter を受け取る形に統一する。

**優先度**: 低（現状動作に問題はないが、設計の一貫性を高めたい場合）

### 6.3 Phase D3: WorldQueryService の二重ファールバック

**問題**: `WorldQueryService` は、`player_location_query_service` 等が `None` の場合にインラインでサブサービスを構築する。一方で `create_world_query_service` は `player_audience_query` のみ明示的に構築し、他は渡していない。結果として、`create_world_query_service` 経由でも `WorldQueryService` 内部の fallback が動く。

**改善案**: `create_world_query_service` 内で `PlayerLocationQueryService`, `SpotContextQueryService`, `AvailableMovesQueryService`, `VisibleContextQueryService` をすべて明示的に構築し、`WorldQueryService` に渡す。`WorldQueryService` の fallback は「テスト用やレガシー呼び出し用」に限定し、本番経路は wiring 経由に統一する。

**優先度**: 低

### 6.4 Phase D4: その他の細かい点

| 対象 | 内容 | 優先度 |
|------|------|--------|
| `PlayerCurrentStateBuilder._get_player_actor` | メソッド内で `WorldObjectId` を import（他と同様にモジュール先頭へ） | 低 |
| `world_simulation_movement_stage_service` | `movement_service_getter: Callable[[], Optional[Any]]` → 型を具体化 | 低 |
| `world_simulation_harvest_stage_service` | `harvest_command_service_getter: Callable[[], Optional[Any]]` → 型を具体化 | 低 |

### 6.5 Phase D の着手タイミング

- **Track C（Domain 系）完了後**にまとめて実施するか、あるいは各 Track の作業中に「ついでに」直す形が現実的。
- 独立した Phase として切る場合は、C1 / C2 完了後を推奨。
- 優先度の高い D1 から順に進め、D2–D4 は余力に応じて対応する。
...
4. **Track D（細かい修正）**: C1 / C2 完了後にまとめて実施するか、各 Track の作業中についでに対応。D1 から順に進める。
