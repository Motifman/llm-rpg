...
- ``EscapeEpisodicStack`` / ``build_escape_episodic_stack`` の **後方互換 alias**
  (既存テスト / 外部依存を壊さないため)
...
# 後方互換: 旧名 (EscapeEpisodicStack / build_escape_episodic_stack) を alias。
# 新規コードは EpisodicStack / build_episodic_stack を直接 import すること。
EscapeEpisodicStack = EpisodicStack
build_escape_episodic_stack = build_episodic_stack
