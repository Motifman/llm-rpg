...
class TestShopEventHandler:
    ...
    def setup_handler(self):
        ...
        return handler, summary_repo, listing_repo

    def test_handle_shop_created(self, setup_handler):
        """ショップ開設イベントでサマリReadModelが作成される"""
        handler, summary_repo, _ = setup_handler
    ...
    def test_handle_shop_created_from_event_without_aggregate(self, setup_handler):
        """イベントペイロードのみで ReadModel を作成できる"""
        handler, summary_repo, _ = setup_handler
        ...
        summary = summary_repo.find_by_id(ShopId(999))
        assert summary is not None
        assert summary.name == "Ghost"

    def test_handle_shop_item_listed(self, setup_handler):
        """出品イベントでリストReadModelが追加され、サマリのlisting_countが増える"""
        handler, summary_repo, listing_repo = setup_handler
        ...
        projection = ShopListingProjection(
            item_name="Test Sword", item_spec_id=5, quantity=3
        )
