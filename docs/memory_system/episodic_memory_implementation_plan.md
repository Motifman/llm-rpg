...
1. `ActionExperienceTrace` / `ObservationExperienceTrace` に、仕様 §2.4 の構造化 location フィールドを**段階的に追加**（文字列 snapshot は維持）。
2. trace 保存処理（例: `agent_orchestrator._append_action_experience_trace`）で `tool_runtime_context` からコピーする。
...
5. **Git 運用**: **小分けコミット・機能単位ブランチ**（巨大ブランチは一度まとめてマージ push 後に転換）。詳細は **§3.1**。
