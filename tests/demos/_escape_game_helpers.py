"""tests/demos/test_escape_game_*.py 用の共有ヘルパー。
...
def create_escape_game_session(
    ...
    *,
    ...
    """escape_game セッションを立ち上げて ``_SessionState`` を返す。
...
def name_to_spot_id(runtime, name: str) -> int:
    ...
    Args:
        runtime: ``EscapeGameRuntime`` または ``runtime._spot_graph_repo``
            を持つオブジェクト。
        name: スポット名 (例: ``"閲覧室"``)。
    """
