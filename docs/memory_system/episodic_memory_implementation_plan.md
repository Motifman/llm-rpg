...
6. `EpisodicChunkCoordinator` がチャンクを評価し、閉じる場合に `ChunkEpisodeDraftBuilder` で `SubjectiveEpisode` を組み立て episode store へ保存する。協調内で `IObservationContextBuffer.drain` → `ISlidingWindowMemory.append_all` を `DefaultPromptBuilder.build` と同順で行う。ルールのみの草案は `ChunkEpisodeDraftBuilder`（`application.llm.services.chunk_episode_draft_builder`）が `ChunkEncodingInput` から組み立て、`interpreted` / `recall_text` は省略可能（後続の LLM で付与）。
