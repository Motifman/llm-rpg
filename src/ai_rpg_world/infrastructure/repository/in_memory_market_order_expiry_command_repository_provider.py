"""CommandScope専用のインメモリ市場注文期限切れrepository provider。"""
...
class InMemoryMarketOrderExpiryCommandRepositoryProvider:
    """同じInMemoryDataStoreの返却先集約をscope内だけ公開する。"""

    def __init__(
        self,
        unit_of_work: InMemoryUnitOfWork,
        context: CommandContext[MarketOrderExpiryCommandRepositoryProviderPort],
    ) -> None:
...
class InMemoryMarketOrderExpiryCommandRepositoryProviderFactory:
    """開始済みインメモリUoWから市場期限切れ用providerを生成する。"""

    def create(
        self,
        context: CommandContext[MarketOrderExpiryCommandRepositoryProviderPort],
        transaction: TransactionPort,
    ) -> InMemoryMarketOrderExpiryCommandRepositoryProvider:
        base_transaction = unwrap_transaction(transaction)
        if not isinstance(base_transaction, InMemoryUnitOfWorkTransactionAdapter):
            raise TypeError(
                "インメモリmarket order expiry providerには"
                "InMemoryUnitOfWorkTransactionAdapterが必要です"
            )
        return InMemoryMarketOrderExpiryCommandRepositoryProvider(
...
__all__ = [
    "InMemoryMarketOrderExpiryCommandRepositoryProvider",
    "InMemoryMarketOrderExpiryCommandRepositoryProviderFactory",
]
