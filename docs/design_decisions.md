...
## 101. outbox一時失敗は順序を保ったまま間隔を広げ、上限で隔離する
...
**関連**: #1094 / #1133 / 判断 #102。
## 104. 時間制限つき妨害は発動・警報・協力解除・締切を別の宣言で持つ
...
## 105. 在室数は用途別の範囲を明示し、動的表示はアプリケーション層で解く
...
## 106. OpenRouter の会話固定 ID は run・世界・player から作り、本文へ混ぜない
...
## 107. 再発動する時限事象は状態を解除し、経過時刻の窓で各周期を識別する
...
## 108. persona の役職知識は世界の事実に限り、立ち回りを代行しない
...
## 109. 人物固有文と役職共通知識を分離し、固定順で system prompt へ連結する
...
## 110. tools payload は状態不変の常在ブロックを先頭へ固定する
...
## 111. 熟考と required が両立しない provider は tool_choice を実験条件として切り替える
...
## 112. 待ち時間の共有単位は役職ではなく interaction が宣言する
