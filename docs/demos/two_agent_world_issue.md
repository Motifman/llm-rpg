...
2. `.env` を準備 (リポジトリには `.env.example` あり、`.env` は git 管理外):
   ...
   # (任意) ツール mode。Issue #155 (TODO 設計再評価) の比較実験用。
   # 既定 (未設定 / "default"): TODO 系ツールを含む従来構成
   # "pure_spot_graph": TODO 系を除外、spot_graph_* + speech のみで実行
   # echo "LLM_TOOL_MODE=pure_spot_graph" >> .env
   ```
...
1. **ターミナル 1 のログ抜粋** (最初の 30 行 + 興味深い箇所): tick 進行 + LLM
   tool 呼び出しの跡が分かるもの
2. **LLM コール回数 と トータル所要時間**: ベースラインは `60 tick / heartbeat
   interval 5 tick × 2 体 ≒ 24 回` (heartbeat 起動のみのケース)。実観測で
   ターンが追加投入されればさらに増える。実値と所要時間を報告してください。
