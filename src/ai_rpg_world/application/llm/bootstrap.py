...
def compose_llm_runtime(
    ...
    **wiring_kwargs: Any,
    ...
    1. create_llm_agent_wiring を呼び observation_registry / llm_turn_trigger を取得する。
    2. composition_builder が渡されていれば、observation_registry を渡して EventHandlerComposition を生成する。
    3. service_builder が渡されていれば、llm_turn_trigger を渡して WorldSimulationApplicationService を生成する。
    ...
    Args:
        ...
        service_builder: llm_turn_trigger を受け取り WorldSimulationApplicationService を返す関数。省略時は None。
    ...
    if service_builder is not None:
        service = service_builder(wiring_result.llm_turn_trigger)
