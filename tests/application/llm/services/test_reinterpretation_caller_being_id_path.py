...
class TestCoordinatorDualPath:
    ...
    def test_caller_being_id_recall_buffer(self) -> None:
        ...
        completion = _StubCompletion({'episode_updates': [{'episode_id': 'e1', 'current_interpretation': 'reinterp text', 'current_recall_text': 'recall text'}]})
        coord = EpisodicReinterpretationCoordinator(episode_store=episodes, recall_buffer_store=setup.recall_buffer, journal_store=setup.journal, completion=completion, turn_interval=1, batch_size=4, max_contexts_per_episode=3)
...
class TestPromptBuilderRecallBufferDualPath:
    ...
    def test_being_id_journal_being_id(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.prompt_builder import _join_passive_recall_texts
    ...
    def test_append_recall_observation_being_id_none_skip(self) -> None:
        ...
        DefaultPromptBuilder._append_recall_observation(builder_no_being, None, observation_obj)
    ...
    def test_being_id_unspecified_recall_text(self) -> None:
        ...
        setup = make_reinterpretation_being_setup()
        cand = EpisodicPassiveRecallCandidate(episode=_ep('e1'), source_axes=('temporal',))
        result = _join_passive_recall_texts(1, (cand,), setup.journal, being_id=None)
