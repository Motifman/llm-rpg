...
かつてここには「将来 EXPLICIT_TARGETS (特定の player に対してのみ観測) を
足す」という予告があったが、**採らないことにした**。解こうとしていた問題
(対象本人にだけ届ける) は ``InteractionDef.notify_target`` が直交する軸として
解いている。本 enum は「第三者に届くか」だけを決め、「対象本人に届くか」は
持たない。1 つの問題に 2 つの仕組みの計画が残るのが最悪の状態なので、予告
ごと消してある (設計 doc §3.6)。
