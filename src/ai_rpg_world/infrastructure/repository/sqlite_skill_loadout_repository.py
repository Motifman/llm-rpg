...
class SqliteSkillLoadoutRepository(SkillLoadoutRepository):
    ...
    def find_by_id(self, entity_id: SkillLoadoutId) -> Optional[SkillLoadoutAggregate]:
        cur = self._conn.execute(
            "SELECT aggregate_blob FROM game_skill_loadouts WHERE loadout_id = ?",
            (int(entity_id),),
        )
        row = cur.fetchone()
        if row is None:
    ...
    def find_all(self) -> List[SkillLoadoutAggregate]:
        cur = self._conn.execute("SELECT aggregate_blob FROM game_skill_loadouts ORDER BY loadout_id ASC")
    ...
    def save(self, entity: SkillLoadoutAggregate) -> SkillLoadoutAggregate:
        self._conn.execute(
            """
            INSERT INTO game_skill_loadouts (loadout_id, owner_id, aggregate_blob)
            VALUES (?, ?, ?)
            ON CONFLICT(loadout_id) DO UPDATE SET
                owner_id = excluded.owner_id,
                aggregate_blob = excluded.aggregate_blob
            """,
            (int(entity.loadout_id), int(entity.owner_id), object_to_blob(entity)),
        )
    ...
    def find_by_owner_id(self, owner_id: int) -> SkillLoadoutAggregate | None:
        cur = self._conn.execute(
            "SELECT aggregate_blob FROM game_skill_loadouts WHERE owner_id = ?",
            (owner_id,),
        )
        row = cur.fetchone()
        if row is None:
