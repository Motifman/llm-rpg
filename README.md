...
## 🔐 環境変数・API Key（LLM 利用時）

LiteLLM で OpenAI 等の API を使う場合、**API Key** を設定する必要があります。

1. `**.env` を使う（ローカル開発）**
  - プロジェクトルートに `.env.example` をコピーして `.env` を作成する。
  - `.env` に `OPENAI_API_KEY=sk-...` を記述する（`sk-` 以降は実際のキーに置き換える）。
  - `.env` は `.gitignore` に含まれておりコミットされない。
  - アプリ起動時に `from dotenv import load_dotenv; load_dotenv()` を 1 回呼ぶと、`.env` の値が環境変数に読み込まれる。
2. **環境変数で直接渡す**
  - シェルで `export OPENAI_API_KEY=sk-...` するか、本番・CI では環境変数として注入する。
3. **コードで渡す**
  - `LiteLLMClient(api_key="sk-...")` のようにコンストラクタで渡す（テストやスタブ差し替え時など）。

**API Key を書く必要がある場所**: 上記のいずれか 1 つでよい。通常は `**.env` ファイル**（リポジトリに含めない）にのみ書き、コード内には書かない。
