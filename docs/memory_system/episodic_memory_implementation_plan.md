...
| tool 実行直後の episode 即保存（旧経路） | 廃止方針 | §2.4・§2.5 のチャンク経路へ統一（後方互換なし）。PR #29 当時の「ツール都度」は orchestrator 側の置換対象 | チャンク完了時にまとめて保存 |
| チャンク境界ルール（第 1 版） | 進行中 | `application.llm.chunk_boundary`・本ブランチ | orchestrator が `explicit_segment_close` 等と組み合わせて呼び出す |
...
6. チャンク完了時にまとめて `SubjectiveEpisode` を作り、episode store に保存する。ルールのみの草案は `ChunkEpisodeDraftBuilder`（`application.llm.services.chunk_episode_draft_builder`）が `ChunkEncodingInput` から組み立て、`interpreted` / `recall_text` は省略可能（後続の LLM で付与）。
