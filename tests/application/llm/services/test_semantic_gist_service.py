...
from ai_rpg_world.application.llm.ports.semantic_gist_completion_port import ISemanticGistCompletionPort
from ai_rpg_world.domain.memory.semantic.value_object.semantic_memory_entry import SemanticMemoryEntry
from ai_rpg_world.application.llm.exceptions import LlmApiCallException
from ai_rpg_world.application.llm.services.semantic_gist_service import SemanticGistResult, SemanticGistService

def _make_episode(*, episode_id: str='ep-1', occurred_at_minute: int=0, recall_text: str='ある記憶', interpreted: str | None=None, prediction_error: str | None=None) -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=episode_id, player_id=1, being_id=BeingId('being_w1_p1'), occurred_at=datetime(2026, 6, 1, 12, occurred_at_minute, tzinfo=timezone.utc), game_time_label=None, source=EpisodeSource(event_ids=('evt-1',)), location=EpisodeLocation(spot_id=3), action=EpisodeAction(tool_name='x'), who=(), what='something happened', why=None, observed='観測本文', expected=None, outcome='ok', prediction_error=prediction_error, felt=None, interpreted=interpreted, cues=(), recall_text=recall_text)
...
class TestSemanticGistServiceGenerate:
    ...
    def test_llm_result_included(self) -> None:
        ...
        port = _StubPort(response={'gist_text': 'タカシは漁の名手で信頼できる', 'importance_score': 8, 'tags': ['タカシ', '信頼']})
        svc = SemanticGistService(port)
        eps = [_make_episode(episode_id='e1', recall_text='タカシが魚を分けてくれた')]
        result = svc.generate(player_name='ハル', persona_block='慎重で寡黙な漁師', cluster_episodes=eps)
    ...
    def test_fifty_over_gist_fifty_truncate(self) -> None:
        ...
        svc = SemanticGistService(port)
        result = svc.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()])
    ...
    def test_importance_score(self) -> None:
        ...
        svc = SemanticGistService(port)
        assert svc.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()]).importance_score == 10
        port2 = _StubPort(response={'gist_text': 'g', 'importance_score': -3, 'tags': []})
        svc2 = SemanticGistService(port2)
        assert svc2.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()]).importance_score == 1
    ...
    def test_importance_score_non_number_default_five(self) -> None:
        ...
        svc = SemanticGistService(port)
        assert svc.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()]).importance_score == 5

    def test_tags_eight_cap(self) -> None:
        """tags は 8件 までで cap。"""
        port = _StubPort(response={'gist_text': 'g', 'importance_score': 5, 'tags': [f't{i}' for i in range(20)]})
        svc = SemanticGistService(port)
        result = svc.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()])
    ...
    def test_tag_element_30_cap(self) -> None:
        """tag の各要素は 30文字で cap。"""
        port = _StubPort(response={'gist_text': 'g', 'importance_score': 5, 'tags': ['あ' * 50]})
        svc = SemanticGistService(port)
        result = svc.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()])
    ...
    def test_tags_empty_string_non_str_element(self) -> None:
        ...
        port = _StubPort(response={'gist_text': 'g', 'importance_score': 5, 'tags': ['ok', '', '  ', 123, None, 'another']})
        svc = SemanticGistService(port)
        result = svc.generate(player_name='x', persona_block='', cluster_episodes=[_make_episode()])
...
class TestSemanticGistServicePromptStructure:
    ...
    def test_player_name_persona_user_included(self) -> None:
        ...
        svc = SemanticGistService(port)
        svc.generate(player_name='ハル', persona_block='慎重で寡黙な漁師', cluster_episodes=[_make_episode(recall_text='魚を獲った')])
    ...
    def test_prediction_error_evidence_user_included(self) -> None:
        ...
        port = _StubPort(response={'gist_text': 'g', 'importance_score': 5, 'tags': []})
        svc = SemanticGistService(port)
        svc.generate(player_name='リン', persona_block='', cluster_episodes=[_make_episode(recall_text='ノアに話しかけた', prediction_error='話せると思ったが無視された')])
    ...
    def test_prediction_error_memory_line(self) -> None:
        ...
        port = _StubPort(response={'gist_text': 'g', 'importance_score': 5, 'tags': []})
        svc = SemanticGistService(port)
        svc.generate(player_name='リン', persona_block='', cluster_episodes=[_make_episode(recall_text='散歩した', prediction_error=None)])
        user_content = port.captured_messages[0][1]['content']
...
class TestSemanticGistServiceErrors:
    ...
    def test_port_llm_api_call_exception(self) -> None:
        ...
        port = _RaisingPort(exc=LlmApiCallException('test fail', error_code='LLM_API_CALL_FAILED'))
        svc = SemanticGistService(port)
