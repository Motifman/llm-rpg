...
from ai_rpg_world.application.llm.tool_constants import TOOL_NAME_MEMO_ADD, TOOL_NAME_MEMO_DONE
...
def _memo_distill_config():
    return episodic_config(belief_evidence_enabled=True, memo_distill_enabled=True)

def _episode(episode_id: str) -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=episode_id, player_id=1, being_id=BeingId('being_w1_p1'), occurred_at=datetime(2026, 7, 26, tzinfo=timezone.utc), game_time_label=None, source=EpisodeSource(event_ids=('evt-1',)), location=EpisodeLocation(), action=EpisodeAction(tool_name='memo_add'), who=(), what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=())
...
class TestWorldRuntimeMemoDistillRewire:
    ...
    def test_flag_off_keeps_transcriber_none(self) -> None:
        runtime = create_world_runtime(_SCENARIO_PATH, config=episodic_config(memo_distill_enabled=False))
    ...
    def test_memo_done_creates_memo_distill_belief_evidence(self) -> None:
        ...
        being_id = runtime._aux_being_resolver.resolve_being_id(runtime._aux_being_default_world_id, PlayerId(1))
        assert being_id is not None
        runtime._episodic_stack.episode_store.put_by_being(being_id, _episode('ep-memo-anchor'))
        add_result = runtime.run_llm_auxiliary_tool(PlayerId(1), TOOL_NAME_MEMO_ADD, {'content': '夜明けに山頂へ向かう計画を維持する'})
        assert add_result.success is True
        memo_id = runtime._todo_store.list_uncompleted_by_being(being_id)[0].id
        done_result = runtime.run_llm_auxiliary_tool(PlayerId(1), TOOL_NAME_MEMO_DONE, {'memo_ids': [memo_id]})
