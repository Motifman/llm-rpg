...
| **PlayerSpokeEvent** | 是 | channel に依存。WHISPER: target_player_id のみ。SAY/SHOUT: 同一 spot_id かつ発言者座標から一定距離以内のプレイヤー | 「〇〇が囁いた/言った/叫んだ: 「…」」 | プレイヤー間発言。SpeechRecipientStrategy で配信先解決。SAY_RANGE=5, SHOUT_RANGE=15（マンハッタン距離）。 |
...
## 8. 観測文の原則（ID 非露出・5W1H・フォールバック）

### 8.1 ID 非露出

- LLM には**内部 ID（数値・UUID）を一切露出しない**。観測プローズ・構造化出力・現在状態・記憶のテキストのいずれにも含めない。
- 名前解決は SpotRepository / PlayerProfileRepository / ItemSpecRepository / ItemRepository 等で行い、**表示名（スポット名・プレイヤー名・アイテム名）のみ**を観測文に含める。

### 8.2 フォールバック時のラベル

名前解決に失敗した場合（リポジトリ未注入・find 未ヒット）は、**数値 ID を使わず**以下の汎用ラベルを用いる。
...
フォールバックが発生する場面は次のとおりである。

- **スポット**: `SpotRepository` が None、または `find_by_id(spot_id)` が None を返したとき。
- **プレイヤー**: `PlayerProfileRepository` が None、または `find_by_id(player_id)` が None、あるいはプロフィールに `name` がないとき。
- **アイテム**: `ItemSpecRepository` / `ItemRepository` が None、または `find_by_id` が None を返したとき。

### 8.3 5W1H と観測文の文体

- 観測文は**LLM プレイヤー視点**で、**何が起きたか**が分かるように書く。ゲーム内部用語（「インタラクションしました」など）は避け、**誰が・何を・どこで・どうした・どうなった**が伝わる自然文にする。
- 例: 「オブジェクトとインタラクションしました」→「宝箱を開けました」「ドアを開きました」「資源を採取しました」など、`InteractionTypeEnum` に応じた具体的な行動として記述する。

---

## 9. 既存観測文の不足点一覧（レジストリにあるイベント）

本節は、観測イベントレジストリに登録済みのイベントについて、**実装前の観測文**が仕様や 5W1H の観点で不十分だった点を記録する。実装で解消した内容も含む。

| イベント | 従来の観測文（例） | 不足点・対応内容 |
|----------|-------------------|------------------|
| **GatewayTriggeredEvent** | 〇〇に到着しました / 誰かがこのスポットに… | スポット名・プレイヤー名はリポジトリで解決。未解決時は「不明なスポット」「不明なプレイヤー」。 |
| **LocationEnteredEvent** | 〇〇に着きました | ロケーション名はイベントの `name` で十分。 |
| **LocationExitedEvent** | ロケーションを出ました | 仕様は「〇〇を出た」だがイベントにロケーション名がないため「ロケーションを出ました」のまま。 |
| **PlayerLocationChangedEvent** | 現在地: 〇〇 / 〇〇がこのスポットに… | スポット名・プレイヤー名を解決。未解決時はフォールバックラベル。 |
| **PlayerDownedEvent** | 戦闘不能になりました / 〇〇が戦闘不能に | プレイヤー名を解決。killer 等は仕様で任意のため今回は未実装可。 |
| **PlayerRevivedEvent** | 復帰しました / 〇〇が復帰しました | 同上。 |
| **PlayerLevelUpEvent** | レベルが上がりました（N→M） | 十分。 |
| **PlayerGoldEarnedEvent** | Nゴールドを獲得しました | 十分。 |
| **PlayerGoldPaidEvent** | Nゴールドを支払いました | 十分。 |
| **ItemTakenFromChestEvent** | チェストからアイテムを取得しました | アイテム名が無い → ItemRepository で解決し「チェストから〇〇を取得しました」。 |
| **ItemStoredInChestEvent** | チェストにアイテムを収納しました | 同上「チェストに〇〇を収納しました」。 |
| **ResourceHarvestedEvent** | （フォーマット未実装） | 追加。「〇〇を採集し、△△を N 個入手しました」。ItemSpecRepository で名前解決。 |
| **SpotWeatherChangedEvent** | 天気が〇〇から〇〇に変わりました | 十分。 |
| **WorldObjectInteractedEvent** | オブジェクトとインタラクションしました | ゲーム用語的 → interaction_type に応じ「宝箱を開けました」「ドアを開きました」「資源を採取しました」等の 5W1H 表現に変更。 |
| **ItemAddedToInventoryEvent** | アイテムを入手しました | アイテム名・数量 → 「〇〇を N 個入手しました」。 |
| **ItemDroppedFromInventoryEvent** | アイテムを捨てました | 「〇〇を捨てました」。 |
| **ItemEquippedEvent** | アイテムを装備しました | 「〇〇を装備しました」。 |
| **ItemUnequippedEvent** | アイテムを外しました | 「〇〇を外しました」。 |
| **InventorySlotOverflowEvent** | インベントリが満杯でアイテムが溢れました | 「インベントリが満杯で〇〇が溢れました」。 |

---

## 10. 今後の作業（別セッションで実施）

以下の作業は、観測レイヤー充実の**次の段階**として別セッションで行う想定である。

1. **レジストリに未登録のイベントの追加**
   - 仕様で「是」「条件付き」だが、現状 `ObservationEventHandlerRegistry` に登録されていないイベント（会話・クエスト・ショップ・モンスター・スキル等）について、配信先解決ロジックの追加 → レジストリへの登録 → `ObservationFormatter` でのフォーマット追加を順に実施する。
2. **WorldObjectInteracted の対象名表示**
   - イベントには `target_id`（WorldObjectId）のみあり、対象の表示名はフォーマッタからは取得していない。必要に応じて PhysicalMapRepository 等で対象オブジェクトの種類・名前を解決し、「〇〇（宝箱）を開けました」のように補足する。
3. **LocationExited の「〇〇を出た」**
   - イベントにロケーション名が無いため、現状は「ロケーションを出ました」。名が必要な場合はイベント拡張またはリポジトリ解決を検討する。
4. **PlayerDowned の「〇〇に倒された」**
   - 仕様では `killer_player_id` があれば「〇〇に倒された」とある。イベントに killer が含まれる場合はフォーマットに反映する。
5. **ブートストラップでのリポジトリ注入**
   - `create_llm_agent_wiring` やアプリのブートストラップで、ObservationFormatter に SpotRepository / PlayerProfileRepository / ItemSpecRepository / ItemRepository を渡し、名前解決を有効にする。
