...
import json
import pytest
from ai_rpg_world.application.being._memory_payload_codecs import afterglow_entry_to_dict, dict_to_afterglow_entry, dict_to_episode_tick_pair, dict_to_recall_slot_entry, episode_tick_pair_to_dict, recall_slot_entry_to_dict
from ai_rpg_world.application.llm.services.afterglow_store import AfterglowEntry, AfterglowSource
from ai_rpg_world.application.llm.services.episodic_recall_slot_store import RecallSlotEntry
...
class TestAfterglowEntryCodec:
    ...
    def test_round_trip_preserves_fields_2(self):
        original = AfterglowEntry(episode_id='ep-002', heading='シキに今日の話をしたが届かなかった', entered_tick=14, source=AfterglowSource.WEAK_RECALL)
    ...
    def test_unknown_source_raises_value_error(self):
        ...
        with pytest.raises((ValueError, KeyError)):
            dict_to_afterglow_entry({'episode_id': 'ep-x', 'heading': 'h', 'entered_tick': 1, 'source': 'not_a_real_source'})
...
class TestBeingMemorySnapshotRecallLayerRoundTrip:
    ...
    def _make_service_with_recall_layer(self):
        ...
        from ai_rpg_world.application.being.being_memory_snapshot_service import BeingMemorySnapshotService
    ...
    def test_round_trip_restores_recall_slot_and_cooldown(self):
        ...
        したら同じ状態に戻る。"""
        from ai_rpg_world.application.llm.services.episodic_recall_slot_store import RecallSlotDecision, RecallSlotEntry
        from ai_rpg_world.domain.being.value_object.being_id import BeingId
        (service, slot, _, _) = self._make_service_with_recall_layer()
        bid = BeingId('being-A')
        slot.replace_all_by_being(bid, slot=(RecallSlotEntry(episode_id='ep-001', entered_tick=3), RecallSlotEntry(episode_id='ep-002', entered_tick=5)), cooldown_until={'ep-old': 15})
    ...
    def test_round_trip_restores_afterglow_index(self):
        ...
        bid = BeingId('being-A')
        new_index = (AfterglowEntry(episode_id='ep-x', heading='海辺で見た光', entered_tick=10, source=AfterglowSource.WEAK_RECALL), AfterglowEntry(episode_id='ep-y', heading='森で薬草を採った', entered_tick=12, source=AfterglowSource.SLOT_EVICTED))
...
class TestGoalEntryCodec:
    ...
    def test_round_trip_preserves_goal_fields(self):
        from datetime import datetime, timezone
        from ai_rpg_world.application.being._memory_payload_codecs import dict_to_goal_entry, goal_entry_to_dict
        from ai_rpg_world.domain.memory.goal.value_object.goal_entry import GoalEntry
        original = GoalEntry(goal_id='goal-1', player_id=1, text='山頂で狼煙を上げて救助される', status='superseded', locked=True, origin='scenario', created_tick=12, created_at=datetime(2026, 7, 1, tzinfo=timezone.utc), supersedes='goal-0')
...
class TestPendingPredictionKindCodec:
    ...
    def test_plan_kind_round_trips(self) -> None:
        from datetime import datetime, timezone
        from ai_rpg_world.application.being._memory_payload_codecs import dict_to_pending_prediction, pending_prediction_to_dict
        from ai_rpg_world.domain.memory.episodic.value_object.pending_prediction import PENDING_KIND_PLAN, PendingPrediction
        original = PendingPrediction(pending_id='pending-1', text='浜を探索し続ければ山頂への道が分かるはず', resolution_cues=('spot:12',), tick_from=20, tick_to=25, origin_episode_id='ep-1', created_tick=15, kind=PENDING_KIND_PLAN)
    ...
    def test_legacy_dict_without_kind_falls_back_promise(self) -> None:
        ...
        from ai_rpg_world.application.being._memory_payload_codecs import dict_to_pending_prediction
        from ai_rpg_world.domain.memory.episodic.value_object.pending_prediction import PENDING_KIND_PROMISE
        legacy = {'pending_id': 'pending-1', 'text': '夕方に木の下でカイトと会う', 'resolution_cues': ['spot:12', 'player:カイト'], 'tick_from': 10, 'tick_to': 15, 'origin_episode_id': 'ep-1', 'created_tick': 10}
