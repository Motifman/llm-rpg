...
3. Phase B3b: `SpotContextQueryService`, `AvailableMovesQueryService` を切り出し
...
**後方互換の扱い（Phase B3a / B3a' で導入した暫定措置）**:
- `WorldQueryService` の `player_location_query_service`, `player_audience_query` を Optional とし、未指定時は内部で生成している
