...
scenario event が全員への警報と8手番後の敗北を担い、同期操作が別室の二人による
解除を担う。同期操作の `on_timeout` は片方が準備した後のやり直しだけに使い、
誰も動かなかった場合を含む全体締切には使わない。
...
- 警報と締切は `FLAG_NOT_SET fuel_restored` を持つ独立した一回限りの event にする
- 解除は異なる二人・別室・3手番未満の同期窓を要求する
