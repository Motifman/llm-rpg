...
class DefaultRecipientStrategy(IRecipientResolutionStrategy):
    ...
    def _resolve_player_downed(self, event: PlayerDownedEvent, add) -> None:
        # 本人は当然観測する。加えて down は物語上の重大状態なので、位置に
        # 関わらず既知の全 player に共有する。詳細な目撃か「遠くの気配」かは
        # formatter が recipient との位置関係から分ける。
        add(event.aggregate_id)
        for pid in self._player_audience_query.all_known_players():
            add(pid)

    def _resolve_player_revived(self, event: PlayerRevivedEvent, add) -> None:
        # down と対称に、復帰も既知の全 player に共有する。遠隔者には formatter
        # が詳細ではなく気配として出す。
        add(event.aggregate_id)
        for pid in self._player_audience_query.all_known_players():
            add(pid)
