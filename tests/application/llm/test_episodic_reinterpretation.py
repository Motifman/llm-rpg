...
from ai_rpg_world.application.llm.ports.episodic_reinterpretation_completion_port import IEpisodicReinterpretationCompletionPort
from ai_rpg_world.domain.being.value_object.being_id import BeingId
...
class _FakeReinterpretationPort(IEpisodicReinterpretationCompletionPort):
    ...
    def complete_episodic_reinterpretation_json(self, messages: list[dict[str, Any]]) -> dict[str, Any]:
...
class _BrokenRecallBufferStore(EpisodicRecallBufferRepository):
    ...
    def append_by_being(self, being_id: BeingId, observation: EpisodicRecallObservation) -> None:
        """常に RuntimeError。"""
        raise RuntimeError('broken')

    def peek_batch_by_being(self, being_id: BeingId, *, batch_size: int, max_contexts_per_episode: int) -> tuple[EpisodicRecallObservation, ...]:
        """常に RuntimeError。"""
        raise RuntimeError('broken')

    def mark_processed_by_being(self, being_id: BeingId, recall_ids: tuple[str, ...]) -> None:
        """常に RuntimeError。"""
        raise RuntimeError('broken')
    ...
    def list_pending_by_being(self, being_id: BeingId) -> list[EpisodicRecallObservation]:
        """常に RuntimeError。"""
        raise RuntimeError('broken')

    def replace_all_pending_by_being(self, being_id: BeingId, observations: list[EpisodicRecallObservation]) -> None:
        """常に RuntimeError。"""
        raise RuntimeError('broken')

    def stamp_prediction_outcome_by_being(self, being_id: BeingId, prediction_context_id: str, prediction_error: str) -> None:
        """常に RuntimeError。"""
        raise RuntimeError('broken')

    def list_episode_ids_by_prediction_context_by_being(self, being_id: BeingId, prediction_context_id: str) -> tuple[str, ...]:
        """常に RuntimeError。"""
        raise RuntimeError('broken')

def _episode(*, episode_id: str, player_id: int=7, being_id: BeingId | None=None, recall_text: str='初期の一人称回想。') -> SubjectiveEpisode:
...
class TestInMemoryEpisodicRecallBufferStore:
    ...
    def test_peek_batch_deduplicates_episode_and_caps_contexts(self) -> None:
        ...
        for i in range(5):
            store.append_by_being(_BEING_7, _recall(recall_id=f'r{i}', episode_id='ep-a', at=base + timedelta(minutes=i), turn_index=i))
        store.append_by_being(_BEING_7, _recall(recall_id='r-b', episode_id='ep-b', at=base + timedelta(minutes=10), turn_index=10))
        batch = store.peek_batch_by_being(_BEING_7, batch_size=1, max_contexts_per_episode=3)
...
class TestEpisodicReinterpretationCoordinator:
    ...
    def test_after_turn_completed_flushes_only_on_tenth_turn(self) -> None:
        ...
        buffer.append_by_being(being_id, _recall(recall_id='r1', episode_id='ep-a', at=datetime(2026, 5, 4, tzinfo=timezone.utc), turn_index=0))
        port = _FakeReinterpretationPort({'episode_updates': [{'episode_id': 'ep-a', 'current_interpretation': '今なら、この扉は単なる障害ではなく合図に見える。', 'current_recall_text': '私はあの古い扉の前で、ただ立ち止まっていたわけではない。冷たい取っ手に触れたとき、閉ざされた先に何かが待つ気配を感じ、慎重になった。今思えば、その重さは道を拒む壁ではなく、私に準備を促す合図だったのだと思う。'}]})
        coord = EpisodicReinterpretationCoordinator(episode_store=episodes, recall_buffer_store=buffer, journal_store=journal, completion=port, turn_interval=10)
        for _ in range(9):
    ...
    def test_partial_llm_updates_mark_only_successful_episode_recalls_processed(self) -> None:
        ...
        journal = setup.journal
        t0 = datetime(2026, 5, 4, tzinfo=timezone.utc)
        buffer.append_by_being(being_id, _recall(recall_id='r-a', episode_id='ep-a', at=t0, turn_index=0))
        buffer.append_by_being(being_id, _recall(recall_id='r-b', episode_id='ep-b', at=t0 + timedelta(minutes=1), turn_index=1))
        port = _FakeReinterpretationPort({'episode_updates': [{'episode_id': 'ep-a', 'current_interpretation': 'ep-a だけ成功。', 'current_recall_text': '私は ep-a の出来事だけを、今の文脈から改めて思い返した。'}]})
        coord = EpisodicReinterpretationCoordinator(episode_store=episodes, recall_buffer_store=buffer, journal_store=journal, completion=port, turn_interval=1)
        assert coord.flush_player(PlayerId(7), being_id) == 1
        assert journal.get_active_by_being(being_id, 'ep-a') is not None
        pending = buffer.peek_batch_by_being(being_id, batch_size=8, max_contexts_per_episode=3)
    ...
    def test_after_turn_completed_does_propagate_sidecar_store_failure(self) -> None:
        ...
        being_id = setup.provision(7)
        episodes = InMemorySubjectiveEpisodeStore()
        episodes.put_by_being(being_id, _episode(episode_id='ep-a', being_id=being_id))
        port = _FakeReinterpretationPort({'episode_updates': []})
        coord = EpisodicReinterpretationCoordinator(episode_store=episodes, recall_buffer_store=_BrokenRecallBufferStore(), journal_store=setup.journal, completion=port, turn_interval=1)
        coord.after_turn_completed(PlayerId(7), being_id)
...
class TestEpisodicReinterpretationCoordinatorErrorDrivenFraming:
    ...
    def test_includes_flag_recall_prediction_outcome_error(self) -> None:
        """flag ON で誤差付き recall は prediction outcome error を含む。"""
        (episodes, setup, being_id) = self._stores()
        buffer = setup.recall_buffer
        buffer.append_by_being(being_id, _recall(recall_id='r1', episode_id='ep-a', at=datetime(2026, 5, 4, tzinfo=timezone.utc), turn_index=0, prediction_context_id='pc-1', prediction_outcome_error='実際には扉は開いていた'))
        port = _FakeReinterpretationPort({'episode_updates': []})
        coord = EpisodicReinterpretationCoordinator(episode_store=episodes, recall_buffer_store=buffer, journal_store=setup.journal, completion=port, turn_interval=1, error_driven_reinterpretation_enabled=True)
    ...
    def test_flag_off_default_system_prompt_payload_before_byte_matches(self) -> None:
        """flag OFF (既定) は U9a 導入前と完全に一致する。"""
        (episodes_off, setup_off, being_id_off) = self._stores()
        buffer_off = setup_off.recall_buffer
        buffer_off.append_by_being(being_id_off, _recall(recall_id='r1', episode_id='ep-a', at=datetime(2026, 5, 4, tzinfo=timezone.utc), turn_index=0, prediction_context_id='pc-1', prediction_outcome_error='実際には扉は開いていた'))
        port_off = _FakeReinterpretationPort({'episode_updates': []})
        coord_off = EpisodicReinterpretationCoordinator(episode_store=episodes_off, recall_buffer_store=buffer_off, journal_store=setup_off.journal, completion=port_off, turn_interval=1, error_driven_reinterpretation_enabled=False)
