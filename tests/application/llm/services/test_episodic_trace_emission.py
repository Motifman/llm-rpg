...
import pytest
from ai_rpg_world.application.llm.contracts.dtos import ActionResultEntry, ToolRuntimeContextDto
...
def _make_episode(*, player_id: int=1, episode_id: str='smoke-ep-1', spot_id: int=3, recall_text: str='RECALL_TEXT_FOR_TEST') -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=episode_id, player_id=player_id, being_id=BeingId(f'being_w1_p{player_id}'), occurred_at=datetime(2026, 5, 1, 12, 0, tzinfo=timezone.utc), game_time_label=None, source=EpisodeSource(event_ids=('evt-1',)), location=EpisodeLocation(spot_id=spot_id), action=EpisodeAction(tool_name='t'), who=('p',), what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=(EpisodicCue(axis='place_spot', value=str(spot_id), source=EpisodicCueSource.RUNTIME_CONTEXT),), recall_text=recall_text)

class TestChunkCoordinatorTraceEmission:
    """chunk 書き込み時に EPISODIC_CHUNK_WRITTEN が記録される。"""

    def _build_coord(self, *, recorder=None, tick: int=5, chunk_subjective_fields_service=None):
    ...
    def _trigger_chunk_close(self, coord, buffer, action_store, player_id: PlayerId, being_id) -> None:
        ...
        action_store.append(player_id, action_summary='wait1', result_summary='ok', occurred_at=t0, occurred_tick=0)
        ...
        action_store.append(player_id, action_summary='wait2', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 1, tzinfo=timezone.utc), occurred_tick=1)
        coord.after_action_recorded(player_id, being_id)
        action_store.append(player_id, action_summary='move', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 2, tzinfo=timezone.utc), occurred_tick=2, scene_boundary=True)
        coord.after_action_recorded(player_id, being_id)
    ...
    def test_recorder_chunk_close_event_emit(self) -> None:
        ...
        chunk_events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_CHUNK_WRITTEN]
        assert len(chunk_events) == 1
        ev = chunk_events[0]
        assert ev.player_id == 1
        assert ev.tick == 5
        assert ev.payload['episode_id']
        assert ev.payload['boundary_reason']
    ...
    def test_naive_aware_datetime_raises_type_error(self) -> None:
        ...
        pid = PlayerId(1)
        action_store.append(pid, action_summary='wait1', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 0, tzinfo=timezone.utc))
        coord.after_action_recorded(pid, being_id)
        buffer.append(pid, ObservationEntry(occurred_at=datetime(2026, 5, 1, 12, 0, 30), output=ObservationOutput(prose='naive obs', structured={'type': 'x'}, observation_category='social', schedules_turn=True), game_time_label=None))
        action_store.append(pid, action_summary='wait2', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 1, tzinfo=timezone.utc))
        coord.after_action_recorded(pid, being_id)
        action_store.append(pid, action_summary='move', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 2, tzinfo=timezone.utc), scene_boundary=True)
        coord.after_action_recorded(pid, being_id)
        assert len(episode_store.list_recent_by_being(being_id, 10)) > 0
    ...
    def test_obs_slice_naive_aware_multiple_sort_does_not_crash(self) -> None:
        ...
        action_store.append(pid, action_summary='wait1', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 0, tzinfo=timezone.utc))
        coord.after_action_recorded(pid, being_id)
        buffer.append(pid, ObservationEntry(occurred_at=datetime(2026, 5, 1, 12, 0, 20), output=ObservationOutput(prose='naive obs 1', structured={'type': 'x'}, observation_category='social', schedules_turn=True), game_time_label=None))
        buffer.append(pid, ObservationEntry(occurred_at=datetime(2026, 5, 1, 12, 0, 40, tzinfo=timezone.utc), output=ObservationOutput(prose='aware obs 2', structured={'type': 'y'}, observation_category='social'), game_time_label=None))
        action_store.append(pid, action_summary='wait2', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 1, tzinfo=timezone.utc))
        coord.after_action_recorded(pid, being_id)
        action_store.append(pid, action_summary='move', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 2, tzinfo=timezone.utc), scene_boundary=True)
        coord.after_action_recorded(pid, being_id)
    ...
    def test_recorder_chunk_raises_exception(self) -> None:
        ...
        class _BrokenRecorder(ITraceRecorder):
            ...
            def close(self):
                pass
        (coord, buffer, action_store, episode_store) = self._build_coord(recorder=_BrokenRecorder())
        pid = PlayerId(1)
        self._trigger_chunk_close(coord, buffer, action_store, pid, being_id)
        assert len(episode_store.list_recent_by_being(being_id, 10)) > 0
...
class TestChunkCoordinatorPredictionOutcomeTraceEmission:
    ...
    def _build_service_with_prediction_error(self, prediction_error):
        ...
        class _StubPort(IEpisodicChunkSubjectiveCompletionPort):
            ...
            def complete_episode_subjective_json(self, messages: list) -> dict:
                return {'interpreted': 'I', 'recall_text': 'R', 'prediction_error': prediction_error}
        return EpisodicChunkSubjectiveFieldsService(_StubPort())

    def test_prediction_error_id_prediction_outcome_rendered(self) -> None:
        """prediction error が確定した瞬間に id 付きで PREDICTION OUTCOME が出る。"""
        recorder = NullTraceRecorder()
        captured = _capture_trace(recorder)
        service = self._build_service_with_prediction_error('鍵がかかっていた')
        (coord, buffer, action_store, episode_store) = self._build_coord(recorder=recorder, chunk_subjective_fields_service=service)
        pid = PlayerId(1)
        t0 = datetime(2026, 5, 1, 12, 0, tzinfo=timezone.utc)
        action_store.append(pid, action_summary='wait1', result_summary='ok', occurred_at=t0, prediction_context_id='predctx-xyz')
        coord.after_action_recorded(pid, being_id)
        buffer.append(pid, ObservationEntry(occurred_at=datetime(2026, 5, 1, 12, 0, 30, tzinfo=timezone.utc), output=ObservationOutput(prose='salient event', structured={'type': 'x'}, observation_category='social', breaks_movement=True), game_time_label=None))
        action_store.append(pid, action_summary='wait2', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 1, tzinfo=timezone.utc), prediction_context_id='predctx-xyz')
        coord.after_action_recorded(pid, being_id)
        action_store.append(pid, action_summary='move', result_summary='ok', occurred_at=datetime(2026, 5, 1, 12, 2, tzinfo=timezone.utc), scene_boundary=True)
        coord.after_action_recorded(pid, being_id)
        outcomes = [e for e in captured if e.kind == TraceEventKind.PREDICTION_OUTCOME]
    ...
    def test_chunk_subjective_fields_service_uninjected_prediction_outcome_not_rendered(self) -> None:
        """同期 merge 自体が走らない (= scheduler 経路のみ使う構成) なら
        sync 側からは emit しない。非同期 scheduler 側が別途担当する。"""
        recorder = NullTraceRecorder()
        captured = _capture_trace(recorder)
        (coord, buffer, action_store, episode_store) = self._build_coord(recorder=recorder, chunk_subjective_fields_service=None)
        pid = PlayerId(1)
        self._trigger_chunk_close(coord, buffer, action_store, pid, being_id)
        outcomes = [e for e in captured if e.kind == TraceEventKind.PREDICTION_OUTCOME]
        assert outcomes == []

    def test_id_off_id_chunk_prediction_outcome_not_rendered(self) -> None:
...
class TestPromptBuilderRecallTraceEmission:
    ...
    def test_recall_candidate_situation_cues_event_included(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.episodic_passive_recall_retrieval import EpisodicPassiveRecallCandidate
        cand = EpisodicPassiveRecallCandidate(episode=ep, source_axes=('place_spot', 'temporal'))
        cue = EpisodicCue(axis='place_spot', value='3', source=EpisodicCueSource.RUNTIME_CONTEXT)
        builder._emit_episodic_recall_trace(player_id=PlayerId(7), situation_cues=(cue,), candidates=[cand])
    ...
    def test_recall_text_chars_total_included(self) -> None:
        ...
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(), candidates=[], relevant_memories_text=joined)
        events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
        assert len(events) == 1
    ...
    def test_recorder_uninjected_emit_op(self) -> None:
        ...
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(cue,), candidates=[])
    ...
    def test_retrieval_debug_payload_included(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.prompt_builder import DefaultPromptBuilder
        from ai_rpg_world.application.llm.services.episodic_passive_recall_retrieval import EpisodicPassiveRecallCandidate, EpisodicPassiveRecallRetrievalDebug
        recorder = NullTraceRecorder()
        captured = _capture_trace(recorder)
        builder = object.__new__(DefaultPromptBuilder)
        builder._trace_recorder = recorder
        ...
        cand_a = EpisodicPassiveRecallCandidate(episode=ep_a, source_axes=('place_spot', 'object'))
        cand_b = EpisodicPassiveRecallCandidate(episode=ep_b, source_axes=('action',))
        debug = EpisodicPassiveRecallRetrievalDebug(raw_row_count_by_axis=(('place_spot', 5), ('object', 3), ('action', 2)), union_episode_count_before_max_cap=4, candidate_episode_sources=(('ep-a', ('place_spot', 'object')), ('ep-b', ('action',))), final_episode_count_by_source_axis=(('action', 1), ('object', 1), ('place_spot', 1)))
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(), candidates=[cand_a, cand_b], retrieval_debug=debug)
        events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
        assert len(events) == 1
        p = events[0].payload
        assert p['raw_row_count_by_axis'] == {'place_spot': 5, 'object': 3, 'action': 2}
        assert p['union_episode_count_before_max_cap'] == 4
        assert p['final_episode_count_by_source_axis'] == {'action': 1, 'object': 1, 'place_spot': 1}
    ...
    def test_habituation_penalty_payload_included(self) -> None:
        ...
        cand = EpisodicPassiveRecallCandidate(episode=ep, source_axes=('place_spot',))
        debug = EpisodicPassiveRecallRetrievalDebug(raw_row_count_by_axis=(('place_spot', 1),), union_episode_count_before_max_cap=1, candidate_episode_sources=(('ep-x', ('place_spot',)),), final_episode_count_by_source_axis=(('place_spot', 1),), habituation_penalty_by_episode=(('ep-x', 3), ('ep-y', 1)))
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(), candidates=[cand], retrieval_debug=debug)
        events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
        assert len(events) == 1
        p = events[0].payload
    ...
    def test_candidate_per_habituation_penalty(self) -> None:
        ...
        ep_a = _make_episode(episode_id='ep-a', recall_text='A')
        ep_b = _make_episode(episode_id='ep-b', recall_text='B')
        cands = [EpisodicPassiveRecallCandidate(episode=ep_a, source_axes=('temporal',)), EpisodicPassiveRecallCandidate(episode=ep_b, source_axes=('temporal',))]
        debug = EpisodicPassiveRecallRetrievalDebug(raw_row_count_by_axis=(('temporal', 2),), union_episode_count_before_max_cap=2, candidate_episode_sources=(('ep-a', ('temporal',)), ('ep-b', ('temporal',))), final_episode_count_by_source_axis=(('temporal', 2),), habituation_penalty_by_episode=(('ep-a', 4),))
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(), candidates=cands, retrieval_debug=debug)
        events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
        assert len(events) == 1
        cands_payload = events[0].payload['candidates']
        assert {c['episode_id']: c['habituation_penalty'] for c in cands_payload} == {'ep-a': 4, 'ep-b': 0}
    ...
    def test_retrieval_debug_unspecified_candidate_habituation_penalty_zero_rendered(self) -> None:
        ...
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(), candidates=[cand])
        events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
    ...
    def test_retrieval_debug_unspecified_existing_payload_compatible(self) -> None:
        ...
        builder._trace_recorder_provider = None
        builder._current_tick_provider = lambda : 1
        builder._emit_episodic_recall_trace(player_id=PlayerId(1), situation_cues=(), candidates=[])
        events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
        assert len(events) == 1
        p = events[0].payload
...
class TestPromptBuilderSectionBreakdownTraceEmission:
    ...
    def test_section_different_char_event_included(self) -> None:
        ...
        tools = [{'type': 'function', 'function': {'name': 'wait', 'description': '待つ', 'parameters': {}}}, {'type': 'function', 'function': {'name': 'move', 'description': '移動する', 'parameters': {}}}]
        builder._emit_prompt_section_breakdown_trace(player_id=PlayerId(3), system_content='SYSTEM' * 10, objective_text='目的目的目的', current_state_text='現在地表示', active_memos_text='メモ', recent_events_text='出来事' * 20, relevant_memories_text='記憶' * 5, inventory_text='持ち物', instruction='指示文', tools=tools, user_content='USER_BODY' * 5)
    ...
    def test_recorder_uninjected_section_breakdown_emit_op(self) -> None:
        ...
        builder._trace_recorder_provider = None
        builder._current_tick_provider = lambda : 0
        builder._emit_prompt_section_breakdown_trace(player_id=PlayerId(1), system_content='', objective_text='', current_state_text='', active_memos_text='', recent_events_text='', relevant_memories_text='', inventory_text='', instruction='', tools=[], user_content='')
...
class TestWorldRuntimeEpisodicTraceE2E:
    ...
    def test_smoke_run_chunk_written_recall_event_emit(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        scenario_path = Path(__file__).resolve().parents[4] / 'data' / 'scenarios' / 'forbidden_library_demo.json'
        ...
        chunk_events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_CHUNK_WRITTEN]
        recall_events = [e for e in captured if e.kind == TraceEventKind.EPISODIC_RECALL]
