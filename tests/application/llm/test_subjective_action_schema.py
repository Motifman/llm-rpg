...
from ai_rpg_world.application.llm.llm_argument_fingerprint import NARRATIVE_ARG_FIELDS
from ai_rpg_world.application.llm.services.tool_catalog.subjective_action import SUBJECTIVE_ACTION_FIELD_PROPERTIES, SUBJECTIVE_ACTION_FIELDS, SUBJECTIVE_ACTION_TEXT_FIELDS, assess_situation_definition, strip_reason_first_action_subjective_schema, with_expected_result_schema, with_subjective_action_schema
EXPECTED_SUBJECTIVE_FIELDS = ('inner_thought', 'intention', 'expected_result', 'emotion_hint')
...
class TestSubjectiveActionFieldSet:
    ...
    def test_text_fields_exclude_emotion_hint_enum(self) -> None:
        ...
        assert SUBJECTIVE_ACTION_TEXT_FIELDS == ('inner_thought', 'intention', 'expected_result')
...
class TestWithSubjectiveActionSchema:
    ...
    def _world_tool(self) -> ToolDefinitionDto:
        return ToolDefinitionDto(name='interact', description='何かに作用する', parameters={'type': 'object', 'properties': {}, 'required': []}, category=ToolCategory.WORLD_ACTION)
...
class TestWithExpectedResultSchema:
    ...
    def _world_tool(self) -> ToolDefinitionDto:
        return ToolDefinitionDto(name='interact', description='何かに作用する', parameters={'type': 'object', 'properties': {'object_label': {'type': 'string'}}, 'required': ['object_label']}, category=ToolCategory.WORLD_ACTION)
    ...
    def test_expected_result_property_matches_canonical_definition(self) -> None:
        ...
        decorated = with_expected_result_schema(self._world_tool(), required=False)
        assert decorated.parameters['properties']['expected_result'] == SUBJECTIVE_ACTION_FIELD_PROPERTIES['expected_result']
...
class TestStripReasonFirstActionSubjectiveSchema:
    ...
    def _action_tool(self) -> ToolDefinitionDto:
        return ToolDefinitionDto(name='interact', description='何かに作用する', parameters={'type': 'object', 'properties': {'object_label': {'type': 'string'}, 'inner_thought': {'type': 'string'}, 'expected_result': {'type': 'string'}, 'goal_update': {'type': ['string', 'null']}, 'goal_outcome': {'type': ['string', 'null'], 'enum': ['achieved', 'abandoned', None]}}, 'required': ['object_label', 'inner_thought', 'expected_result']}, category=ToolCategory.WORLD_ACTION)
...
class TestWithGoalUpdateSchema:
    ...
    def _todo_tool(self) -> ToolDefinitionDto:
        return ToolDefinitionDto(name='todo_add', description='メモ追加', parameters={'type': 'object', 'properties': {}, 'required': []}, category=ToolCategory.WORLD_ACTION)

    def test_goal_update_added_to_properties_not_required(self) -> None:
        from ai_rpg_world.application.llm.services.tool_catalog.subjective_action import with_goal_update_schema
...
class TestWithGoalOutcomeSchema:
    ...
    def test_goal_outcome_added_as_nullable_enum_not_required(self) -> None:
        from ai_rpg_world.application.llm.services.tool_catalog.subjective_action import with_goal_outcome_schema
    ...
    def test_description_distinguishes_close_from_rephrase(self) -> None:
        ...
        from ai_rpg_world.application.llm.services.tool_catalog.subjective_action import with_goal_outcome_schema
        desc = with_goal_outcome_schema(self._world_tool()).parameters['properties']['goal_outcome']['description']
