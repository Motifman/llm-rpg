...
def _chunk_episode_id(
    *,
    prose: str = "砂浜には何もなかった",
    action: ActionResultEntry | None = None,
) -> str:
    ...
    return ChunkEpisodeDraftBuilder().build(inp).episode_id
...
def _action_episode_id(**overrides: Any) -> str:
    ...
    args = {
        ...
        "occurred_at": _T0,
        "tool_name": "interact",
        "canonical_arguments": {"intention": "箱の中身を確かめる"},
        "runtime_context": ToolRuntimeContextDto(targets={}, current_spot_id=7),
...
class TestRenderingDoesNotChangeIdentity:
    ...
    def test_rewording_a_tool_message_keeps_the_action_episode_id(self) -> None:
        ...
        before = _action_episode_id(
            command_result=LlmCommandResultDto(success=True, message="開封に成功した。"),
        )
        after = _action_episode_id(
            command_result=LlmCommandResultDto(success=True, message="蓋が開いた。"),
        )
...
class TestIdentityStillSeparatesDifferentEvents:
    ...
    def test_success_and_failure_are_different_events(self) -> None:
        ...
        failed = LlmCommandResultDto(
            success=False, message="開封に成功した。", error_code="LOCKED",
        )
