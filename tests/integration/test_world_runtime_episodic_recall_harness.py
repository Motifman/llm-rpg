"""world_runtime の典型シナリオを模した episodic recall 検証ハーネス (Issue #283 後続)。

目的:
- world_runtime demo は現在 episodic memory pipeline を wire していないが、
...
- このハーネスでは、world_runtime の典型シナリオ (リンが閲覧室→入口広間→書架A
...
SPOT_SHELF_B = 5
PLAYER_LIN = 2
BEING_LIN = BeingId(f'being_w1_p{PLAYER_LIN}')
PLAYER_KAITO = 1

def _make_visit_episode(*, episode_id: str, player_id: int, spot_id: int, spot_name: str, occurred_at: datetime, what: str='そのスポットに居た', outcome: str='観察した') -> SubjectiveEpisode:
    ...
    """
    cue_place = EpisodicCue(axis='place_spot', value=str(spot_id), source=EpisodicCueSource.RUNTIME_CONTEXT)
    return SubjectiveEpisode(episode_id=episode_id, player_id=player_id, being_id=BeingId(f'being_w1_p{player_id}'), occurred_at=occurred_at, game_time_label=None, source=EpisodeSource(event_ids=(f'evt-{episode_id}',)), location=EpisodeLocation(spot_id=spot_id), action=EpisodeAction(tool_name='travel_to'), who=(f'player_{player_id}',), what=what, why=None, observed=spot_name, expected=None, outcome=outcome, prediction_error=None, felt=None, interpreted=None, cues=(cue_place,), recall_text=f'[tick episode] {spot_name} を訪れた: {what}')
...
def lin_visit_history() -> InMemorySubjectiveEpisodeStore:
    ...
    episodes = [('ep1-library', SPOT_LIBRARY_HALL, '閲覧室', '覚書を読んだ', base), ('ep2-entry', SPOT_ENTRY_HALL, '入口広間', '案内板を見た', base + timedelta(minutes=5)), ('ep3-shelfA', SPOT_SHELF_A, '書架A', '本を examine し『水』の断片語を見つけた', base + timedelta(minutes=10)), ('ep4-entry-return', SPOT_ENTRY_HALL, '入口広間', '通り抜けた', base + timedelta(minutes=14)), ('ep5-library-return', SPOT_LIBRARY_HALL, '閲覧室', '戻ってきた', base + timedelta(minutes=16))]
    for (ep_id, spot_id, spot_name, what, occurred_at) in episodes:
        store.put_by_being(BEING_LIN, _make_visit_episode(episode_id=ep_id, player_id=PLAYER_LIN, spot_id=spot_id, spot_name=spot_name, what=what, occurred_at=occurred_at))
    return store

def _runtime_context_at(spot_id: int) -> ToolRuntimeContextDto:
    """『現在 spot_id にいる』だけが分かる最小 runtime_context。"""
    return ToolRuntimeContextDto(targets={}, current_spot_id=spot_id, current_sub_location_id=None)

class TestRecallByCurrentLocationCue:
    """シナリオ 1: 同一スポットへの再訪。place_spot 軸で過去訪問が recall される。"""

    def test_returns_recall(self, lin_visit_history: InMemorySubjectiveEpisodeStore) -> None:
        ...
        cues = build_situation_episodic_cues(runtime_context=_runtime_context_at(SPOT_LIBRARY_HALL), observation_structured=None, latest_action=None)
        assert ('place_spot', '2') in {(c.axis, c.value) for c in cues}
        result = svc.retrieve(being_id=BEING_LIN, situation_cues=cues, limit_per_axis=10, max_candidates=10)
...
class TestRecallByObservationStructuredCue:
    ...
    def test_observation_structured_included_recall(self, lin_visit_history: InMemorySubjectiveEpisodeStore) -> None:
        ...
        """
        svc = EpisodicPassiveRecallRetrievalService(lin_visit_history)
        observation_structured = {'type': 'speech_message', 'speaker_player_id': PLAYER_KAITO, 'spot_id_value': SPOT_SHELF_A, 'content': 'リン、書架A で待ってるよ'}
        cues = build_situation_episodic_cues(runtime_context=_runtime_context_at(SPOT_LIBRARY_HALL), observation_structured=observation_structured, latest_action=None)
...
class TestRecallByFreeTextMention:
    ...
    def test_matcher_uninjected_cue(self, lin_visit_history: InMemorySubjectiveEpisodeStore) -> None:
        """matcher を渡さなければ自由文経路は無効 = 旧挙動を維持する後方互換。
        ``WorldNounMatcher`` を wire しない demo / scenario で動作が
        変わらないことを担保。"""
        svc = EpisodicPassiveRecallRetrievalService(lin_visit_history)
        observation_structured = {'type': 'speech_message', 'speaker_player_id': PLAYER_KAITO, 'spot_id_value': SPOT_LIBRARY_HALL, 'content': 'リン、書架A で待ってるよ'}
        cues = build_situation_episodic_cues(runtime_context=_runtime_context_at(SPOT_LIBRARY_HALL), observation_structured=observation_structured, latest_action=None)
        cue_keys = {(c.axis, c.value) for c in cues}
        assert ('place_spot', '3') not in cue_keys

    def test_matcher_cue_episode_recall(self, lin_visit_history: InMemorySubjectiveEpisodeStore) -> None:
        """**Issue #283 後続の主目的動作**:
        ...
        from ai_rpg_world.application.llm.services.world_noun_matcher import WorldNounMatcherBuilder
        matcher = WorldNounMatcherBuilder().add_spot('入口広間', spot_id=SPOT_ENTRY_HALL).add_spot('閲覧室', spot_id=SPOT_LIBRARY_HALL).add_spot('書架A', spot_id=SPOT_SHELF_A).add_spot('書架B', spot_id=SPOT_SHELF_B).build()
        svc = EpisodicPassiveRecallRetrievalService(lin_visit_history)
        observation_structured = {'type': 'speech_message', 'speaker_player_id': PLAYER_KAITO, 'spot_id_value': SPOT_LIBRARY_HALL, 'content': 'リン、書架A で待ってるよ'}
        observation_prose = '〈解読室の扉〉の向こうから、カイトの遠くの声が聞こえる: 「リン、書架A で待ってるよ」'
        cues = build_situation_episodic_cues(runtime_context=_runtime_context_at(SPOT_LIBRARY_HALL), observation_structured=observation_structured, latest_action=None, observation_prose=observation_prose, noun_matcher=matcher)
        cue_keys = {(c.axis, c.value) for c in cues}
        assert ('place_spot', '3') in cue_keys
        result = svc.retrieve(being_id=BEING_LIN, situation_cues=cues, limit_per_axis=10, max_candidates=10)
        ids = {c.episode.episode_id for c in result.candidates}
...
class TestRecallScalesWithRepeatedVisits:
    ...
    def test_two_distinct_episode_recall(self, lin_visit_history: InMemorySubjectiveEpisodeStore) -> None:
