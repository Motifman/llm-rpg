"""Issue #227 PR 1: world_runtime runtime の observation resolver 配線テスト。

第13/14回実験で観測された「PlayerSpokeEvent が遠距離まで届く」根本原因の
1 つは、world_runtime の resolver が ``SpotGraphRecipientStrategy`` 1 つしか
登録していなかったため、`PlayerSpokeEvent` (strategy_key=\"speech\") が
配信先解決の対象外になっていたことだった。

本テストは ``create_world_runtime`` で組み立てた runtime の
...
class TestWorldRuntimeObservationResolverWiring:
    """create_world_runtime が組み立てた resolver が全 strategy を持つ。"""
