...
# 2. <redacted-internal>.* を含むなら非 0
echo "HostName <redacted-host>" > leak.md
git add leak.md
if bash "$SCRIPT" >/dev/null 2>&1; then
  echo "FAIL: <redacted-internal> FQDN was not flagged" >&2
  exit 1
fi

# 3. tesla<redacted-internal>* も非 0
rm leak.md
echo "ProxyCommand ssh <redacted-jump-host> -W %h:%p" > leak.md
git add leak.md
if bash "$SCRIPT" >/dev/null 2>&1; then
  echo "FAIL: tesla.<redacted-internal> FQDN was not flagged" >&2
  exit 1
fi
