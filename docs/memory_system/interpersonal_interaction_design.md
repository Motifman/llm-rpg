...
| **対人行為** | **書けない。本 doc の対象** |
| 陣営の勝敗 | 全滅は書ける (`SURVIVING_PLAYERS_WITH_STATE_AT_MOST`)。得点制などは未対応 |
| 会議・投票 | 書けない (優先度低) |
...
**結果**: 対人行為 (PR 1-8) の完了後、陣営の勝敗のうち「全滅」は
`SURVIVING_PLAYERS_WITH_STATE_AT_MOST` で書けるようにした。残るのは
**会議・投票**である。これはフェーズの状態遷移 (招集 → 議論 → 投票 → 追放)
と、その間のプレイヤー行動の制限が要るので、本 doc の延長ではなく別設計になる。

陣営条件で決めたこと:

- 「生存」から外すのは `PlayerOutcomeEnum.DEAD` が確定した相手だけ。倒れている
  (`is_down`) だけの相手は蘇生できるので生存として数える。ここで終わらせると
  駆けつけて助け起こす行為の意味が消える
- 結果は集団としての `LOSE`。「襲う側の勝ち」という読み替えはシナリオの語りが
  担い、エンジンは陣営ごとの勝敗を持たない。陣営別の結果を持たせるのは、
  会議・投票と同じく別設計の話になる
- 判定材料 (役割 = `PlayerStatusAggregate.state`、生死 = outcome registry) が
  渡っていなければ例外にする。黙って未成立にすると、勝敗が永久に確定しないまま
  実験が走り続け、「まだ続いている」と区別が付かない§4 の △ 3 件 (脅す・手当ての即時回復・盗みの所持確認) と ✕ 2 件 (口封じ・
