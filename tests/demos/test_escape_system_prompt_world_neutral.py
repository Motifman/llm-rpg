...
def _build(*, has_goal: bool) -> str:
    return build_escape_system_prompt(
        world_title="テスト世界",
...
class TestBuildEscapeSystemPromptGoalFraming:
    """build_escape_system_prompt の goal/escape 前提が has_goal で切り替わる。"""
    ...
    def test_default_is_has_goal_true(self) -> None:
        ...
        assert _build(has_goal=True) == build_escape_system_prompt(
            world_title="テスト世界",
            persona_block="【ペルソナ】テスト",
            safe_intro="導入文",
            participant_names=(),
