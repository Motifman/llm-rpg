...
class TestErrorGatedEncodingFlagOff:
    ...
    def test_flag_off_with_salience_enabled_keeps_uniform_length_instruction(self) -> None:
        """salience_enabled=True でも error_gated_encoding_enabled=False なら
        recall_text の長さ指示は U6 導入時の一律指示 (250〜450 字) のまま。"""
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'salience': 'low'})
        svc = EpisodicChunkSubjectiveFieldsService(port, salience_enabled=True, error_gated_encoding_enabled=False)
        svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
...
class TestErrorGatedEncodingFlagOn:
    ...
    def test_flag_on_with_salience_enabled_uses_salience_conditional_length(self) -> None:
        """flag ON かつ salience_enabled=True のとき、recall_text の長さ指示が
        salience=high/low の連動指示になる。"""
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'salience': 'high'})
        svc = EpisodicChunkSubjectiveFieldsService(port, salience_enabled=True, error_gated_encoding_enabled=True)
        svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
        sys_content = _system_content(port)
    ...
    def test_flag_on_without_salience_enabled_keeps_uniform_length_instruction(self) -> None:
        ...
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r'})
        svc = EpisodicChunkSubjectiveFieldsService(port, salience_enabled=False, error_gated_encoding_enabled=True)
        svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
        assert _system_content(port) == _SYSTEM_EPISODE_SUBJECTIVE_JSON
    ...
    def test_constructor_type_error_for_non_bool(self) -> None:
        ...
        with pytest.raises(TypeError, match='error_gated_encoding_enabled'):
            EpisodicChunkSubjectiveFieldsService(port, error_gated_encoding_enabled='yes')
