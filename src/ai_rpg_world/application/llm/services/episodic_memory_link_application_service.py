...
class EpisodicMemoryLinkApplicationService:
    ...
    def on_episode_committed(
        ...
        *,
        ...
        if newest.episode_id != episode.episode_id:
            return
        self._ensure_capacity_before_link(
            being_id, pid, newest.episode_id, prev.episode_id, now
        )
    ...
    def on_passive_recall_candidates(
        ...
        *,
        ...
        self._bump_recall_counts(being_id, capped, now)
        for i in range(len(capped)):
            for j in range(i + 1, len(capped)):
                self._ensure_capacity_before_link(
                    being_id, player_id, capped[i], capped[j], now
                )
    ...
    def _put_episode(self, being_id: BeingId, episode: SubjectiveEpisode) -> None:
        """being_id 経路で episode を upsert する。"""
    ...
    def _bump_recall_counts(
        ...
        for eid in episode_ids:
            ep = self._get_episode(being_id, eid)
            if ep is None:
                continue
            updated = replace(
                ep,
