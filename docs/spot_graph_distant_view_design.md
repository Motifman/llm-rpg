...
この絞り込みは、静的 area だけでなく active な `distant_cues` も合算した「遠景候補集合」全体に適用する。焚き火や煙が複数同時に出ても、cue 経路だけが方角集約や件数上限を素通りして prompt を膨らませることは許さない。
...
同じ除外規則は `distant_cues` にも適用する。自分が今いる area や隣接 area を source とする cue は、遠景ではなく局所説明・オブジェクト説明・行動結果の領分として扱う。
...
ambient 系の trace は毎ターン全エージェントで出すと trace を膨らませる。`DISTANT_VIEW_RENDERED` / `DISTANT_VIEW_SKIPPED` は capture flag または debug profile 限定にする。状態変化系の `DISTANT_CUE_STATE_CHANGED` / `DISTANT_CUE_DELIVERED` は、世界変化と記憶流入の根拠になるため常時記録する。
...
- active な `distant_cues` に静的 area と別枠の小さな上限を設けるか。初期方針では静的 area と cue を合算して共通上限にかけるが、煙や光だけを優先表示したい scenario が出たら再検討する。
