...
**Phase C1c 設計上の注意**:
- **コンストラクタの大きな変更は別タスク**: `MonsterAggregate.__init__` は現状、リポジトリや既存呼び出しとの後方互換のため、個別パラメータ（`behavior_state`, `behavior_target_id`, `behavior_last_known_position` 等）を受け取り、内部で `MonsterBehaviorState.from_legacy()` と `FeedMemory` を構築している。コンストラクタの引数を `MonsterBehaviorState` や `FeedMemory` を直接受け取る形に変える **破壊的 API 変更** は、リポジトリのシリアライズや全生成箇所の修正を伴うため、別 Phase として保留。
- **`from_legacy` の意味**: `MonsterBehaviorState.from_legacy()` は「旧形式の個別フィールド（state, target_id, last_known_position 等）から値オブジェクトを構築する」ファクトリメソッド。ドメインモデル本体（`_behavior_state` の型は `MonsterBehaviorState`）は新形式であり、`_behavior_target_id` や `_behavior_last_known_position` の個別フィールドは廃止済み。`from_legacy` は後方互換のための**変換層**であり、レガシーな実装がドメインに残っているわけではない。
