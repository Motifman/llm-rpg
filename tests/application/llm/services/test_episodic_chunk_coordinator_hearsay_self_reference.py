...
def _build_coord(*, player_name_provider):
    ...
    port = _StubPort({'interpreted': 'I', 'recall_text': 'R', 'heard_claims': [{'speaker': _PLAYER_NAME, 'claim': '自分の発言'}, {'speaker': 'リオ', 'claim': '北の泉は安全だ'}]})
    subjective_service = EpisodicChunkSubjectiveFieldsService(port, hearsay_enabled=True)
    coord = EpisodicChunkCoordinator(observation_buffer=buffer, short_term_memory=sliding, action_result_store=action_store, episodic_episode_store=episode_store, chunk_episode_draft_builder=ChunkEpisodeDraftBuilder(), chunk_subjective_fields_service=subjective_service, player_name_provider=player_name_provider)
...
class TestEpisodicChunkCoordinatorHearsaySelfReferenceSyncPath:
    ...
    def test_player_name_provider_self_speaker_claim_rejected(self) -> None:
        ...
        (coord, buffer, action_store, episode_store, being_id) = _build_coord(player_name_provider=lambda pid: _PLAYER_NAME)
    ...
    def test_player_name_provider_unwired_self_speaker(self) -> None:
        ...
        """
        (coord, buffer, action_store, episode_store, being_id) = _build_coord(player_name_provider=None)
        player_id = PlayerId(1)
        _trigger_chunk_close(coord, buffer, action_store, player_id, being_id)
        episodes = episode_store.list_recent_by_being(being_id, limit=10)
        assert len(episodes) == 1
...
class TestEpisodicChunkCoordinatorInitValidation:
    ...
    def test_player_name_provider_callable_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match='player_name_provider'):
            EpisodicChunkCoordinator(observation_buffer=buffer, short_term_memory=sliding, action_result_store=action_store, episodic_episode_store=episode_store, chunk_episode_draft_builder=ChunkEpisodeDraftBuilder(), player_name_provider='not-callable')
