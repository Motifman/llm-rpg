...
1. **ObservationRecipientResolver** のインターフェースと、イベント種別ごとのルール実装（本仕様の表をルールソースとする）。
2. **IObservationFormatter** のインターフェースと、イベント種別＋注意レベルごとの文言・スキップ判定。
3. プレイヤーごとの**コンテキストバッファ**への蓄積と、LLM 呼び出し時の「現在状態 ＋ 直近観測」の結合。
4. 上記に従い、**観測用イベントハンドラ**を EventPublisher に登録し、既存の UoW と同期/非同期の使い分けに合わせて実装する。
