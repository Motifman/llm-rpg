...
from ai_rpg_world.application.llm.services.episodic_chunk_subjective_fields import _SYSTEM_EPISODE_SUBJECTIVE_JSON, EpisodicChunkSubjectiveFieldsService, _normalize_heard_claims
...
class TestNormalizeHeardClaims:
    ...
    def test_multiple_valid_claims_parsed(self) -> None:
        out = _normalize_heard_claims([{'speaker': 'リオ', 'claim': '岩礁海岸は山に通じていない'}, {'speaker': 'エイダ', 'claim': '北の泉は安全だ'}])
    ...
    def test_entries_missing_speaker_or_claim_are_dropped(self) -> None:
        ...
        out = _normalize_heard_claims([{'claim': '話者不明の主張'}, {'speaker': 'リオ'}, {'speaker': '  ', 'claim': '空白話者'}, {'speaker': 'カイ', 'claim': '有効'}])
        assert len(out) == 1
    ...
    def test_placeholder_speakers_are_dropped(self) -> None:
        ...
        out = _normalize_heard_claims([{'speaker': '不明', 'claim': '水場は東にある'}, {'speaker': '誰か', 'claim': '北は危ない'}, {'speaker': 'Unknown', 'claim': 'x'}, {'speaker': 'リオ', 'claim': '有効な伝聞'}])
...
class TestHearsayFlagOff:
    ...
    def test_heard_claims_ignored_when_disabled(self) -> None:
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'heard_claims': [{'speaker': 'リオ', 'claim': '何か'}]})
...
class TestHearsayFlagOn:
    ...
    def test_claims_land_on_episode(self) -> None:
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'heard_claims': [{'speaker': 'リオ', 'claim': '岩礁海岸は山に通じていない'}]})
        svc = EpisodicChunkSubjectiveFieldsService(port, hearsay_enabled=True)
        merged = svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
...
class TestNormalizeHeardClaimsSelfReference:
    ...
    def test_claim_from_actor_itself_is_dropped(self) -> None:
        ...
        """
        out = _normalize_heard_claims([{'speaker': 'カイト', 'claim': '自分でそう言った'}, {'speaker': 'リオ', 'claim': '北の泉は安全だ'}], actor_name='カイト')
        assert [c.speaker for c in out] == ['リオ']

    def test_matching_is_case_and_whitespace_insensitive(self) -> None:
        """actor_name/speaker の前後空白や大小文字の違いは同一人物として判定する。"""
        out = _normalize_heard_claims([{'speaker': '  Kaito  ', 'claim': '自分の発言'}], actor_name='kaito')
        assert out == ()
    ...
    def test_actor_name_none_does_not_filter_anyone(self) -> None:
        ...
        """
        out = _normalize_heard_claims([{'speaker': 'カイト', 'claim': '何か言った'}], actor_name=None)
    ...
    def test_merge_llm_subjective_fields_filters_self_referential_claim(self) -> None:
        ...
        enc = _make_encoding()
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r', 'heard_claims': [{'speaker': 'カイト', 'claim': '自分の発言'}, {'speaker': 'リオ', 'claim': '北の泉は安全だ'}]})
        svc = EpisodicChunkSubjectiveFieldsService(port, hearsay_enabled=True)
        merged = svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc, actor_name='カイト')
...
class TestNormalizeHeardClaimsRunawayGuard:
    ...
    def test_claims_beyond_limit_are_dropped_and_warned(self, caplog) -> None:
        ...
        (黙って切り捨てない = 観測可能性を保つ)。
        """
        claims = [{'speaker': f'話者{i}', 'claim': f'主張{i}'} for i in range(5)]
    ...
    def test_claim_text_beyond_char_limit_is_truncated_and_warned(self, caplog) -> None:
        ...
        long_claim = 'あ' * 500
        with caplog.at_level('WARNING'):
            out = _normalize_heard_claims([{'speaker': 'リオ', 'claim': long_claim}])
        assert len(out) == 1
    ...
    def test_claims_within_limits_produce_no_warning(self, caplog) -> None:
        ...
        with caplog.at_level('WARNING'):
            out = _normalize_heard_claims([{'speaker': 'リオ', 'claim': '短い主張'}])
        assert len(out) == 1
