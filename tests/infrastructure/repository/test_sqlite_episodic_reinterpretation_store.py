...
class TestSqliteEpisodicReinterpretationStore:
    ...
    def test_recall_buffer_roundtrip_and_mark_processed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.append(_recall("r1", "ep-a", base))
            store.append(_recall("r2", "ep-a", base + timedelta(minutes=1)))
            del store

            reopened = SqliteEpisodicReinterpretationStore.connect(path)
            batch = reopened.peek_batch(7, batch_size=8, max_contexts_per_episode=1)
            assert [r.recall_id for r in batch] == ["r1"]
            assert reopened.pending_count(7) == 2
            reopened.mark_processed(7, ("r1",))
            assert reopened.pending_count(7) == 1
    ...
    def test_put_active_supersedes_previous_after_reopen(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reopened = SqliteEpisodicReinterpretationStore.connect(path)
            active = reopened.get_active(7, "ep-a")
            assert active is not None
            assert active.entry_id == "j2"
            history = reopened.list_by_episode(7, "ep-a")
