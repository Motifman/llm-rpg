...
from ai_rpg_world.application.llm.services.episodic_chunk_subjective_fields import _build_system_prompt, EpisodicChunkSubjectiveFieldsService
...
def _make_encoding(player_id: int=1) -> Any:
    ...
    act = ActionResultEntry(occurred_at=t, action_summary='扉を開けた', result_summary='ok', tool_name='world_no_op', success=True)
...
def _user_content(port: _StubSubjectivePort) -> str:
    ...
    return next((m['content'] for m in port.last_messages if m.get('role') == 'user'), '')

def _system_content(port: _StubSubjectivePort) -> str:
    assert port.last_messages is not None
    return next((m['content'] for m in port.last_messages if m.get('role') == 'system'), '')
...
class TestUnconsciousContextFlagOff:
    ...
    def test_does_not_call_provider_never(self) -> None:
        ...
        svc = EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider=_provider, unconscious_context_enabled=False)
    ...
    def test_system_prompt_user_message_before_byte_matches(self) -> None:
        ...
        svc_with = EpisodicChunkSubjectiveFieldsService(port_with, salience_enabled=True, unconscious_context_provider=lambda pid, cues: '無視されるはずのテキスト', unconscious_context_enabled=False)
        ...
        assert _system_content(port_with) == _build_system_prompt(salience_enabled=True, unconscious_context_enabled=False)
...
class TestUnconsciousContextFlagOn:
    ...
    def test_provider_returned_section_included(self) -> None:
        ...
        svc = EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider=_provider, unconscious_context_enabled=True)
    ...
    def test_system_prompt(self) -> None:
        ...
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r'})
        svc = EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider=lambda pid, cues: '- なにか (確信度: 0.50)', unconscious_context_enabled=True)
        svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
    ...
    def test_returns_section_not_rendered_provider_empty_string_when(self) -> None:
        ...
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r'})
        svc = EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider=lambda pid, cues: '', unconscious_context_enabled=True)
        svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
        user_content = _user_content(port)
    ...
    def test_returns_section_not_rendered_provider_none_when(self) -> None:
        ...
        draft = ChunkEpisodeDraftBuilder().build(enc, being_id=BeingId('being-test'))
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r'})
        svc = EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider=lambda pid, cues: None, unconscious_context_enabled=True)
        svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
        user_content = _user_content(port)
        assert '## いまの自分（信念と自己像）' not in user_content
    ...
    def test_provider_chunk_raises_exception(self) -> None:
        ...
        svc = EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider=_raising_provider, unconscious_context_enabled=True)
...
class TestUnconsciousContextRuleFieldGuard:
    ...
    def test_llm_raises_value_error(self) -> None:
        ...
        class _RuleTamperingPort(IEpisodicChunkSubjectiveCompletionPort):
            ...
            def complete_episode_subjective_json(self, messages: list[dict[str, Any]]) -> dict[str, Any]:
                return {'interpreted': '信念に沿って解釈した', 'recall_text': 'r', 'prediction_error': '信念どおりだったので驚かなかった'}
        svc = EpisodicChunkSubjectiveFieldsService(_RuleTamperingPort(), unconscious_context_provider=lambda pid, cues: '- 罠が多い (確信度: 0.90)', unconscious_context_enabled=True)
        merged = svc.merge_llm_subjective_fields(draft, persona_text='', encoding_input=enc)
...
class TestUnconsciousContextTypeValidation:
    ...
    def test_unconscious_context_provider_callable_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError):
            EpisodicChunkSubjectiveFieldsService(port, unconscious_context_provider='not-callable')

    def test_unconscious_context_enabled_bool_raises_type_error(self) -> None:
        """unconscious context enabled が bool でなければ TypeError。"""
        port = _StubSubjectivePort({'interpreted': 'i', 'recall_text': 'r'})
        with pytest.raises(TypeError):
            EpisodicChunkSubjectiveFieldsService(port, unconscious_context_enabled='yes')
