"""LiteLLM 経由の Episode Encoder 用ポート（ツール無し・テキスト JSON）。"""
...
class LiteLlmEpisodeEncodingLlmPort(IEpisodeEncodingLlmPort):
    """`litellm.completion` で system/user のみ送信。`response_format` は OpenAI / 一部プロバイダ対応。"""
    ...
    def complete(
        ...
        *,
        ...
        try:
            ...
            self._logger.exception("LiteLLM episode encoding completion failed: %s", e)
