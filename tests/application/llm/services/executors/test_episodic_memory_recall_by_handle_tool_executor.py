...
from ai_rpg_world.application.llm.services.executors.episodic_memory_recall_by_handle_tool_executor import EpisodicMemoryRecallByHandleToolExecutor, FORGOTTEN_MESSAGE
...
def _setup():
    ...
    ep = SubjectiveEpisode(episode_id=_EPISODE_ID, player_id=_PLAYER_ID, being_id=BeingId('being_w1_p7'), occurred_at=datetime(2026, 7, 1, tzinfo=timezone.utc), game_time_label=None, source=EpisodeSource(event_ids=('evt-1',)), location=EpisodeLocation(), action=EpisodeAction(tool_name='t'), who=('p',), what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=(), recall_text='水の断片語を見つけた手応えがあった。', heading=_HEADING)
    episode_store.put_by_being(being_id, ep)
    afterglow_store.apply_decision(being_id, (AfterglowEntry(episode_id=_EPISODE_ID, heading=_HEADING, entered_tick=10, source=AfterglowSource.WEAK_RECALL),))
    executor = EpisodicMemoryRecallByHandleToolExecutor(episode_store=episode_store, afterglow_store=afterglow_store, slot_store=slot_store, slot_capacity=4, current_tick_provider=lambda : 15)
...
class TestRecallByHandleSuccess:
    ...
    def test_returns_recall_text_with_heading_prefix(self) -> None:
        ...
        result = executor._run(acting, {'handle': make_afterglow_handle(_EPISODE_ID)})
    ...
    def test_inserts_into_slot_with_current_tick(self) -> None:
        ...
        executor._run(acting, {'handle': make_afterglow_handle(_EPISODE_ID)})
        ids = [e.episode_id for e in slot_store.get_slot(being_id)]
        assert _EPISODE_ID in ids
        entry = next((e for e in slot_store.get_slot(being_id) if e.episode_id == _EPISODE_ID))
...
class TestRecallByHandleFailures:
    ...
    def test_forgotten_handle_returns_forgotten_message(self) -> None:
        ...
        (executor, acting, _, _, _, _) = _setup()
        result = executor._run(acting, {'handle': 'ep_deadbeef'})
        assert result.success is True
