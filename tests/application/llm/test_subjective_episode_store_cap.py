...
def _episode(pid: int, eid: str, occurred_at: datetime, cue_value: str) -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=eid, player_id=pid, being_id=BeingId(f'being_w1_p{pid}'), occurred_at=occurred_at, game_time_label=None, source=EpisodeSource(event_ids=(f'evt-{eid}',)), location=EpisodeLocation(), action=EpisodeAction(tool_name='t'), who=('p',), what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=(EpisodicCue(axis='topic', value=cue_value, source=EpisodicCueSource.OBSERVATION_FREETEXT),), recall_text='...')
...
class TestFifoEviction:
    ...
    def test_evict_episode_cue_index_removed(self) -> None:
        ...
        results = store.list_by_cue_by_being(being_id, EpisodicCue(axis='topic', value='topic_a', source=EpisodicCueSource.OBSERVATION_FREETEXT), limit=10)
        assert results == []
