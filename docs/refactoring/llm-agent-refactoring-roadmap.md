...
- [x] **v1.2 完了**（2026-03-14 main マージ: 6-stage facade / monster policy separation / regression harness）
- [x] **follow-up 完了**（2026-03-15: constructor slim 化 / monster context 分離 / stage heavy logic 抽出）
...
**進捗管理プラン**: [world-simulation-service-refactoring-plan.md](./world-simulation-service-refactoring-plan.md)

**現状**:
- `WorldSimulationApplicationService` は 6 stage へ委譲する facade として維持
- `WorldSimulationCollaboratorFactory` により collaborator の default 組み立てを facade 外へ移設済み
- monster context/feed/target、environment effect、hit box update、spawn slot は専用 collaborator に分離済み
- world simulation + wiring integration の代表 157 件で回帰確認済み
...
- [x] **Phase B3a, B3a', B3b, B3c 実施済み**（2026-03-15: PlayerLocationQueryService, SpotContextQueryService, AvailableMovesQueryService, VisibleContextQueryService の切り出し）
- [x] **Phase B3d, B3e 実施済み**（2026-03-15: PlayerRuntimeContextBuilder の facade 化、create_world_query_service ファクトリの追加）
- **対象**: `world_query_service.py` (約430行), `player_current_state_builder.py` (約340行)
- **テスト**: `test_world_query_service.py` (約1058行), `test_player_current_state_builder.py`

**進捗管理プラン**: [phase2-3-refactoring-plan.md](./phase2-3-refactoring-plan.md) Phase B3 参照
...
**分割案**:
- 以下に分離:
  - `PlayerLocationQueryService`（`_get_player_location_impl` を移行）
  - `SpotContextQueryService`（`_get_spot_context_for_player_impl` を移行）
  - `AvailableMovesQueryService`（`_get_available_moves_impl` を移行）
  - `VisibleContextQueryService`（`build_visible_context` を移行）
  - `PlayerRuntimeContextBuilder`（既存 `PlayerSupplementalContextBuilder` を位置づけまたはファサードとして活用）
  - `create_world_query_service(...)` ファクトリ（新規クエリサービス群の組み立てを一箇所に集約）
- `WorldQueryService` は薄い facade に留める
- 観測系の `PlayerAudienceQueryService.players_at_spot` を world 系でも再利用（同スポット人数の重複解消）
...
- [x] **Phase C1a 実施済み**（2026-03-15: MonsterBehaviorStateMachine の切り出し、遷移規則の集約）
...
- `docs/refactoring/world-simulation-service-refactoring-plan.md` - WorldSimulationService 専用の進捗管理プラン（v1.2 完了後の follow-up 管理）
...
### 2026-03-15: WorldSimulationService の現在地を再調査

- **内容**: `main` 上の `v1.2` milestone 完了を確認し、1.1 は「未着手」ではなく「tick facade extraction / monster policy separation / regression harness 完了済み」の状態へ更新。未使用 helper と facade の組み立て責務を今後の follow-up として切り出し、専用の進捗管理プランを追加
- **関連項目**: 1.1
- **備考**: 進捗管理は [world-simulation-service-refactoring-plan.md](./world-simulation-service-refactoring-plan.md) に移行
...
| 2026-03-15 | 1.1 の現在地を再調査し、`v1.2` 完了を反映。world-simulation-service-refactoring-plan.md を追加し、今後の follow-up 進捗管理先を明確化 |
