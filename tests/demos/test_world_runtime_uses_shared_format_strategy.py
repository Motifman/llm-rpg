"""Issue #227 chore β: world_runtime の prompt と本家 format strategy の整合確認。

PR で world_runtime の ``build_full_prompt`` の section 組み立てを本家
...
class TestWorldRuntimeUsesSharedFormatStrategy:
    """world_runtime runtime の prompt が本家 strategy 形式 (``【...】``) を使う。"""
    ...
    def test_world_runtime_prompt_body_is_produced_by_shared_strategy(self) -> None:
        """world_runtime の prompt body と shared strategy 出力が完全一致する。

        runtime が strategy に渡している素材を再現して直接 strategy を呼び、
        prompt["user"] から指示文を除いた本体と byte 単位で一致することを確認する。
        これにより将来 world_runtime 側が独自に section を組み直すような変更が
        ...
        instruction = WorldRuntime._ESCAPE_GAME_ACTION_INSTRUCTION
        ...
        # PR-B 以降は WorldRuntime._ESCAPE_GAME_OBJECTIVE_TEXT (旧ハードコード)
