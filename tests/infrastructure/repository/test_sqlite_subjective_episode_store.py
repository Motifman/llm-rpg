...
def _episode(*, episode_id: str='ep-1', player_id: int=7, occurred_at: datetime | None=None, cues: tuple[EpisodicCue, ...] | None=None, recall_text: str='r', co_present: tuple[str, ...]=()) -> SubjectiveEpisode:
    ts = occurred_at or datetime(2026, 5, 3, 12, 0, tzinfo=timezone.utc)
    cue_list = cues or (EpisodicCue(axis='place_spot', value='12', source=EpisodicCueSource.RUNTIME_CONTEXT),)
    return SubjectiveEpisode(episode_id=episode_id, player_id=player_id, being_id=BeingId(f'being_w1_p{player_id}'), occurred_at=ts, game_time_label=None, source=EpisodeSource(event_ids=('evt-a',)), location=EpisodeLocation(), action=EpisodeAction(tool_name='t'), who=('p',), co_present=co_present, what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=cue_list, recall_text=recall_text)
...
class TestSqliteSubjectiveEpisodeStoreBasics:
    ...
    def test_list_by_cue_after_reopen(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            path = str(Path(tmp) / 'episodes.db')
            cue = EpisodicCue(axis='place_spot', value='12', source=EpisodicCueSource.RUNTIME_CONTEXT)
            store = SqliteSubjectiveEpisodeStore.connect(path)
...
class TestSqliteSubjectiveEpisodeReplaceAll:
    ...
    def test_replace_all_cue_index(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            new_cue = EpisodicCue(axis='place_spot', value='999', source=EpisodicCueSource.RUNTIME_CONTEXT)
