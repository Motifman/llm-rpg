"""survival_island_v2 シナリオで episodic memory pipeline が
...
`create_world_runtime` 内で `LLM_EPISODIC_ENABLED=1` のとき自動的に
`build_episodic_stack` が呼ばれる。つまり**コード変更なしで survival シナリオ
にエピソード記憶が使えるはず**、というのが本テストの確認対象。

# 検証

1. `LLM_EPISODIC_ENABLED` 未設定 → `_episodic_stack is None`
2. `LLM_EPISODIC_ENABLED=1` → stack が組み立てられる
