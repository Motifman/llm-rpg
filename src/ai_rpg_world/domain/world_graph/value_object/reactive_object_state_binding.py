...
注意 — 同一 object/同一 key を複数 binding が書く場合の挙動:
旧 same-key-set 制約は副次的に「同じ object の同じキーを 2 binding が
持つこと」を見つけにくくしていた。撤廃後はキー重複がより起こりやすく
なるため、stage が iteration 順で上書き適用する（**last-write-wins**、
エラーにならない）点に注意。複数 binding が同じキーを管理したい場合は、
シナリオ作家が責任を持って「相互排他的な predicate」を用いるか、
ドメインキーを分けること。フレームワーク側で cross-binding バリデーション
は行わない（コストが見合わない / 別 lint pass の責務）。
"""
...
class ReactiveObjectStateBinding:
    ...
    # 著者が宣言した観測テキスト。flip 方向ごとに別文を出せる。None なら
    # その方向への state 変化は observation を発火させない (silent)。
    # 例: 採取資源の cooldown reset (false→true) には
    # `narrative_on_true="ベリーの茂みに新しい実が生っているのが見える"` を渡す。
    narrative_on_true: Optional[str] = None
    narrative_on_false: Optional[str] = None
