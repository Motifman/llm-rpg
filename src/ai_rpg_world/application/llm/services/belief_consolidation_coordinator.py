...
【目的への前進評価 (reflect)】
ユーザメッセージに「現在の目的」が渡っている場合、この期間の evidence 群を
その目的と照らし、**目的に向けて前進があったか**を判断すること。明らかに
停滞している (目的に一歩も近づいていない / 同じ場所を空回りしている) と
判断したときだけ、次の形の決定を 1 つだけ足す:
{"action": "reflect", "verdict": "stalled", "statement": "<停滞の内省を一人称で 1 文>"}
前進している / 判断がつかないときは reflect を書かない。reflect は belief を
作らない (意識に「気づき」を返すだけ)。待ち合わせや看病のように、動いて
いないことに理由がある場合は停滞ではない。"""
