...
#: 過去 run の snapshot にそのまま入っている形 (接尾辞なし)。
...
def _episode(episode_id: str, what: str) -> SubjectiveEpisode:
    return SubjectiveEpisode(
        ...
        location=EpisodeLocation(spot_id=42, tile_area_ids=()),
...
class TestRecallByHandleWorksForBothVersions:
    ...
    def test_each_handle_resolves_to_its_own_episode(self) -> None:
        ...
        for episode_id in (_OLD_ID, _NEW_ID):
            prefix = resolve_episode_id_prefix_from_handle(
                make_afterglow_handle(episode_id)
            )
            matches = [
                episode
                for episode in store.list_recent_by_being(being, limit=10)
                if episode.episode_id.startswith(prefix)
            ]
