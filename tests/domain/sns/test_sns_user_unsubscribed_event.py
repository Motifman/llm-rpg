...
from src.domain.sns.base_sns_event import SnsUserUnsubscribedEvent
...
class TestSnsUserUnsubscribedEvent:
    ...
    def test_create_unsubscribed_event_success(self):
        ...
        event = SnsUserUnsubscribedEvent(
    ...
    def test_create_unsubscribed_event_with_default_time(self):
        """デフォルト時間での購読解除イベント作成テスト"""
        subscriber_user_id = 1
        subscribed_user_id = 2
    ...
    def test_invalid_from_user_id_raises_error(self):
        ...
        with pytest.raises(ValueError, match="from_user_id must be positive"):
            SnsUserUnsubscribedEvent(from_user_id=0, to_user_id=2)

    def test_invalid_to_user_id_raises_error(self):
        """無効な対象IDの場合のエラーテスト"""
        with pytest.raises(ValueError, match="to_user_id must be positive"):
            SnsUserUnsubscribedEvent(from_user_id=1, to_user_id=0)

    def test_equality_comparison(self):
        """等価性の比較テスト"""
        time = datetime(2023, 1, 1, 12, 0, 0)
        event1 = SnsUserUnsubscribedEvent(from_user_id=1, to_user_id=2, created_at=time)
        event2 = SnsUserUnsubscribedEvent(from_user_id=1, to_user_id=2, created_at=time)
        assert event1 == event2
        assert hash(event1) == hash(event2)

    def test_auto_generated_time_is_recent(self):
        """自動生成された時間が現在時刻に近いことを確認"""
        event = SnsUserUnsubscribedEvent(from_user_id=1, to_user_id=2)
        now = datetime.now()
