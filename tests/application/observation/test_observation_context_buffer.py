...
class TestDefaultObservationContextBuffer:
    ...
    def test_append_rejects_bad_runtime_context_type(self, buffer, sample_entry):
        """runtime_context が ToolRuntimeContextDto でないとき TypeError"""
        with pytest.raises(TypeError, match="runtime_context must be ToolRuntimeContextDto"):
            buffer.append(PlayerId(1), sample_entry, runtime_context="nope")  # type: ignore[arg-type]
