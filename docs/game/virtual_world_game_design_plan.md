...
- **キャラクターの認識**: 記憶喪失。ここに来る前のことは一切覚えていない。閉じ込められていることは自覚している
...
> **NOTE**: ストーリーの詳細は外部 AI サービスと共にブラッシュアップ予定。
> 事前情報は [story_concept_consultation_brief.md](./story_concept_consultation_brief.md) にまとめてある。

**荘園住人から見たプレイヤー:**
- 「天の声」「夢の向こうの人」のような存在
- 正体不明だが、敵意はないと認識されている
...
```
                    ...
                    │ 統合              │              ↑
                    │                   │       脱出成功 or 時間切れ
                    │ [転送開始]         │──→ ワールド選択 ──→ キャラ選択 ──→ ゲーム画面
                    │ [キャラ管理]       │──→ 荘園に新住人が「現れる」演出
                    └───────────────────┘
```
...
**Phase 2 移行後:**
タイトル画面は初回起動時のみ表示。以降は荘園画面がロビーを兼ねる。
...
- 右側: スロット（最大5枠）。選択するとキャラアイコン+名前がスロットに入る
...
| ゲーム終了（脱出成功） | 画面が明るくなる → パーティクル → リザルトへフェード | 2.5s |
| ゲーム終了（時間切れ） | 画面が暗くなる → ノイズエフェクト → リザルトへフェード | 2.0s |
