...
## v108 vLLM 接続（推奨: SSH 専用 Host `v108-vllm`）

**ホストの追加は Mac 側の `~/.ssh/config` に数行書くだけ**で、v108 サーバ側の設定変更は不要です。
`LocalForward` は SSH クライアントが「ローカル 18001 への接続を v108 の 8001 に転送する」機能で、エイリアス（別名）のようなものです。

`~/.ssh/config` に以下があること（この Mac には追加済み）:

```sshconfig
Host v108-vllm
     HostName v108.fun.bio.keio.ac.jp
     ProxyCommand ssh tesla.fun.bio.keio.ac.jp -W %h:%p
...
```bash
make vllm-tunnel    # 未接続なら ssh -N v108-vllm を起動
