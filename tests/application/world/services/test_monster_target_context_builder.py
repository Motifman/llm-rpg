...
class TestMonsterTargetContextBuilder:
    ...
    def test_builds_context_with_both_aggro_and_pack_leader_target(self):
        """aggro_store と pack_leader_target_id が両方ある場合は両方を含む context を返す"""
        aggro_store = mock.Mock(
            get_threat_by_attacker=mock.Mock(return_value={WorldObjectId(99): 10})
        )
        leader = mock.Mock()
        leader.object_id = WorldObjectId(2)
        leader.component.is_pack_leader = True
        ...
        physical_map = mock.Mock(
            spot_id=mock.sentinel.spot_id,
            get_actors_in_pack=mock.Mock(return_value=[leader]),
        )
    ...
    def test_returns_none_for_non_autonomous_actor(self):
        """AutonomousBehaviorComponent 以外の actor は None を返す"""
        from ai_rpg_world.domain.world.entity.world_object_component import ActorComponent

        aggro_store = mock.Mock(get_threat_by_attacker=mock.Mock(return_value={WorldObjectId(99): 5}))
        builder = MonsterTargetContextBuilder(
            monster_repository=mock.Mock(),
            aggro_store=aggro_store,
        )
        ...
        assert builder.build_target_context(actor, physical_map, WorldTick(10)) is None
