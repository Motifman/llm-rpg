...
from ai_rpg_world.application.being.capture_being_snapshot_to_file_use_case import BeingNotFoundForSnapshotError, CaptureBeingSnapshotToFileUseCase
from ai_rpg_world.application.being.restore_being_snapshot_from_file_use_case import RestoreBeingSnapshotFromFileUseCase
...
def _make_environment():
    ...
    memory_snapshot = BeingMemorySnapshotService(memo_store=memo, semantic_store=semantic, memory_link_store=link, recall_buffer_store=recall, reinterpretation_journal_store=journal, episodic_episode_store=episode, recall_slot_store=InMemoryEpisodicRecallSlotStore(), afterglow_store=InMemoryAfterglowStore(), recall_habituation_store=InMemoryEpisodicRecallHabituationStore(), belief_evidence_buffer_store=InMemoryBeliefEvidenceBufferStore(), recall_success_store=InMemoryEpisodicRecallSuccessStore(), pending_prediction_store=InMemoryPendingPredictionStore(), goal_journal_store=InMemoryGoalJournalStore(), stagnation_pressure_store=InMemoryStagnationPressureStore())
    gateway = BeingSnapshotFileGateway()
    capture = CaptureBeingSnapshotToFileUseCase(being_repository=repo, memory_snapshot_service=memory_snapshot, file_gateway=gateway)
    restore = RestoreBeingSnapshotFromFileUseCase(being_repository=repo, memory_snapshot_service=memory_snapshot, file_gateway=gateway)
    return {'repo': repo, 'memo': memo, 'semantic': semantic, 'link': link, 'recall': recall, 'journal': journal, 'episode': episode, 'capture': capture, 'restore': restore}

def _make_being() -> Being:
    return Being(being_id=BeingId('being_w1_p1'), identity=BeingIdentity(name='アダ', first_person='わたし'), attachment=BeingAttachment(world_id=WorldId(1), player_id=PlayerId(1)), declared_memory_kinds=[MemoryKind.MEMO, MemoryKind.SEMANTIC])
...
class TestCaptureUseCase:
    ...
    def test_writes_repo_does_not_change_capture_file(self, tmp_path: Path) -> None:
...
class TestRestoreUseCase:
    ...
    def test_capture_restore_memory_restored(self, tmp_path: Path) -> None:
    ...
    def test_payload_snapshot_restore_memory(self, tmp_path: Path) -> None:
        ...
        env = _make_environment()
        snap = BeingSnapshot(being_id_value='being_w1_p1', identity_name='アダ', identity_first_person='わたし', attachment_world_id=1, attachment_player_id=1, declared_memory_kinds=('memo',), snapshot_version=2, memory_payload_json=None)
...
class TestConstructorTypeGuards:
    ...
    def test_capture_constructor_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match='being_repository'):
            CaptureBeingSnapshotToFileUseCase(being_repository='bad', memory_snapshot_service=_make_environment()['capture']._memory, file_gateway=BeingSnapshotFileGateway())
    ...
    def test_restore_constructor_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match='file_gateway'):
            RestoreBeingSnapshotFromFileUseCase(being_repository=InMemoryBeingRepository(), memory_snapshot_service=_make_environment()['capture']._memory, file_gateway='bad')

    def test_execute_argument_raises_type_error(self, tmp_path: Path) -> None:
        """execute の引数型違反。"""
        env = _make_environment()
        with pytest.raises(TypeError):
            env['capture'].execute('ada', tmp_path / 'x.json')
        with pytest.raises(TypeError):
            env['capture'].execute(BeingId('x'), 'string-not-path')
        with pytest.raises(TypeError):
            env['restore'].execute('string-not-path')
