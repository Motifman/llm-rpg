...
#: 内部識別子らしい並び。error_code は英大文字とアンダースコアで書かれる。
_INTERNAL_IDENTIFIER = re.compile(r"[A-Z][A-Z_]{5,}")

#: 実 run で証拠になった code (実測: 全 42 run で 50 件)。
#: 言い換えが無いと、この 2 つが今そのまま漏れている。
_CODES_SEEN_IN_RUNS = (
...
def _episode(occurred_at: datetime) -> SubjectiveEpisode:
    return SubjectiveEpisode(
        ...
        action=EpisodeAction(tool_name="interact"),
        who=(),
        what="w",
        why=None,
        observed="o",
...
def _transcribe(error_code: str, *, tool_name: str = "interact", count: int = 3) -> str:
    ...
    evidence = transcriber.record_if_triggered(
        being_id, tool_name=tool_name, error_code=error_code, count=count
    )
...
class TestEvidenceTextHidesInternalIdentifiers:
    ...
    def test_no_phrasing_contains_an_internal_identifier(
        self, error_code: str
    ) -> None:
...
class TestPhrasingTableCoversWhatRunsProduce:
    ...
    def test_describe_is_stable_for_the_same_input(self) -> None:
        ...
        first = describe_repeated_failure(
            tool_name="interact", error_code="INVALID_TARGET_LABEL", count=3
        )
        second = describe_repeated_failure(
            tool_name="interact", error_code="INVALID_TARGET_LABEL", count=3
        )
