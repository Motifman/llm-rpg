...
class SqliteSkillDeckProgressRepository(SkillDeckProgressRepository):
    ...
    def find_by_id(self, entity_id: SkillDeckProgressId) -> Optional[SkillDeckProgressAggregate]:
        cur = self._conn.execute(
            "SELECT aggregate_blob FROM game_skill_deck_progresses WHERE progress_id = ?",
            (int(entity_id),),
        )
        row = cur.fetchone()
        if row is None:
    ...
    def find_all(self) -> List[SkillDeckProgressAggregate]:
        cur = self._conn.execute(
            "SELECT aggregate_blob FROM game_skill_deck_progresses ORDER BY progress_id ASC"
        )
    ...
    def save(self, entity: SkillDeckProgressAggregate) -> SkillDeckProgressAggregate:
        self._conn.execute(
            """
            INSERT INTO game_skill_deck_progresses (progress_id, owner_id, aggregate_blob)
            VALUES (?, ?, ?)
            ON CONFLICT(progress_id) DO UPDATE SET
                owner_id = excluded.owner_id,
                aggregate_blob = excluded.aggregate_blob
            """,
            (int(entity.progress_id), int(entity.owner_id), object_to_blob(entity)),
        )
    ...
    def find_by_owner_id(self, owner_id: int) -> SkillDeckProgressAggregate | None:
        cur = self._conn.execute(
            "SELECT aggregate_blob FROM game_skill_deck_progresses WHERE owner_id = ?",
            (owner_id,),
        )
        row = cur.fetchone()
        if row is None:
