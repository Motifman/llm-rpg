...
from ai_rpg_world.application.llm.services.state_collapse_evidence_transcriber import StateCollapseEvidenceTranscriber
...
from ai_rpg_world.domain.memory.semantic.value_object.belief_evidence import BELIEF_EVIDENCE_SALIENCE_HIGH
from ai_rpg_world.domain.memory.semantic.value_object.belief_evidence_source_kind import BeliefEvidenceSourceKind

def _episode(episode_id: str, *, occurred_at: datetime, being_id: BeingId = BeingId('being-1')) -> SubjectiveEpisode:
    return SubjectiveEpisode(episode_id=episode_id, player_id=1, being_id=being_id, occurred_at=occurred_at, game_time_label=None, source=EpisodeSource(event_ids=('evt-1',)), location=EpisodeLocation(), action=EpisodeAction(tool_name='spot_graph_interact'), who=(), what='w', why=None, observed='o', expected=None, outcome='ok', prediction_error=None, felt=None, interpreted=None, cues=())
...
class TestStateCollapseEvidenceTranscriberIndependentDedupTracking:
    ...
    def test_dedup_state_is_tracked_per_being_id(self) -> None:
        ...
        for bid in (being_a, being_b):
            episode_store.put_by_being(bid, _episode(f'ep-{bid.value}', occurred_at=datetime(2026, 7, 1, tzinfo=timezone.utc), being_id=bid))
        transcriber = StateCollapseEvidenceTranscriber(buffer_store, episode_store)
...
class TestStateCollapseEvidenceTranscriberTrace:
    ...
    def test_emits_belief_evidence_trace_event_for_down(self) -> None:
        ...
        transcriber = StateCollapseEvidenceTranscriber(buffer_store, episode_store, trace_recorder_provider=lambda : recorder, current_tick_provider=lambda : 7)
        transcriber.record_down_evidence(being_id)
    ...
    def test_emits_belief_evidence_trace_event_for_hunger_max(self) -> None:
        ...
        recorder = NullTraceRecorder()
        captured = _capture_trace(recorder)
        transcriber = StateCollapseEvidenceTranscriber(buffer_store, episode_store, trace_recorder_provider=lambda : recorder, current_tick_provider=lambda : 9)
        transcriber.record_hunger_max_evidence(being_id)
        events = [e for e in captured if e.kind == TraceEventKind.BELIEF_EVIDENCE]
        assert len(events) == 1
...
class TestStateCollapseEvidenceTranscriberTypeGuards:
    ...
    def test_rejects_non_repository_episode_store(self) -> None:
        with pytest.raises(TypeError):
            StateCollapseEvidenceTranscriber(InMemoryBeliefEvidenceBufferStore(), object())

    def test_rejects_non_being_id_for_down(self) -> None:
        transcriber = StateCollapseEvidenceTranscriber(InMemoryBeliefEvidenceBufferStore(), InMemorySubjectiveEpisodeStore())
        with pytest.raises(TypeError):
