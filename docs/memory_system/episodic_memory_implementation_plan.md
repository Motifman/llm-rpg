...
| tool 実行直後の episode 即保存（`ActionEpisodeDraftBuilder` 単発経路） | 廃止済み | `LlmAgentOrchestrator` から削除。保存は `EpisodicChunkCoordinator` → `ChunkEpisodeDraftBuilder` のチャンク完了時のみ | — |
| チャンク境界協調・オーケストレータ配線（第 1 版） | 完了 | `application.llm.services.episodic_chunk_coordinator`・`feature/episodic-chunk-orchestrator-wiring`。`IObservationContextBuffer` / `ISlidingWindowMemory` / `IActionResultStore` / `IEpisodicEpisodeStore` を注入し、`decide_chunk_boundary` で判定。現状 `run_turn` 内の tool 成功後は `explicit_segment_close=True`（1 tool/turn MVP で区間を閉じる） | 複数 tool を同一セグメントに溜める制御は別タスク |
...
| `recall_text` | prompt に入れる短文 | template or LLM 生成 | LLM 適用前・チャンク草案段階では `None` にしてよい（受動想起連結は空をスキップ） |
...
**方針更新（2026-05）**: エピソード保存は **チャンク経路**に統一する（ツール都度の即保存は後方互換なく廃止）。入力契約は `application.llm.contracts.chunk_encoding` の `ChunkEncodingInput`。**区間内に少なくとも 1 件の `ActionResultEntry` があるときだけ**エピソード生成を起動できる。観測は材料・境界ヒントとし、直近出来事テキストと同一規則の `merge_observations_and_action_results_to_unified_timeline`（`DefaultRecentEventsFormatter` と同じ行整形・`occurred_at` 昇順）で行動結果と統合する。境界のたび、`IObservationContextBuffer.drain` → `ISlidingWindowMemory.append_all` を **`DefaultPromptBuilder.build` と同順**で行い、観測の一次情報をプロンプトと揃える（手順は `observation_sliding_window_sync.drain_observation_buffer_into_sliding_window` と等価で、`EpisodicChunkCoordinator` は協調内で同じ 2 呼び出しを行う）。

補足（材料の揃い方）:

- チャンク内の各ツールについて、`LlmAgentOrchestrator` 上で tool 名・args・runtime context・result がまとまることは引き続き材料の起点となる。
- いつチャンクを閉じエピソード生成へ進むかの純粋ルールは `application.llm.chunk_boundary`（`decide_chunk_boundary`, `summarize_observation_boundary_hints`）。境界ヒントの隣接比較は `occurred_at` 昇順（観測タプルの並びは任意）。区間に行動が足りるかのゲートは `chunk_encoding.chunk_encoding_episode_generation_allowed` のみ真実（第 1 版は `ActionResultEntry` が 1 件以上）。
- 観測だけで episode を作る経路は後段に回す（チャンク第 1 版も観測のみ区間では起動しない）。

### 2.5 いつ保存するか

**方針更新（2026-05）**: チャンク境界で材料が揃い、`ChunkEncodingInput` からエンコードが許可される場合に `SubjectiveEpisode` を組み立て episode store へ保存する。

チャンク内の各ツールについて、従来どおり次の順で記録が進む:
...
6. `EpisodicChunkCoordinator` がチャンクを評価し、閉じる場合に `ChunkEpisodeDraftBuilder` で `SubjectiveEpisode` のルールフィールドだけを組み立て、その後オプションの `EpisodicChunkSubjectiveFieldsService`（`application.llm.services.episodic_chunk_subjective_fields`）が `IEpisodicChunkSubjectiveCompletionPort` 経由で JSON 応答 **`interpreted` / `recall_text` のみ**をマージしてから episode store に保存する。協調内で `IObservationContextBuffer.drain` → `ISlidingWindowMemory.append_all` を `DefaultPromptBuilder.build` と同順で行う。`create_llm_agent_wiring` / `create_spot_graph_wiring` は `LiteLLMClient` または明示 `episodic_chunk_subjective_completion=` があれば上記サービスを注入し、それ以外（Stub）は未付与のまま。LLM が失敗・不正 JSON のときサービス側は草案の `what` / `observed` 由来のテンプレへフォールバックする（`interpreted` / `recall_text` が未設定とは限らなくなる）。

失敗した tool call もチャンク材料に含められる。むしろ `expected_result` と失敗結果の差は記憶として重要である。
...
- `EpisodicCue`
- `EpisodeSource`
- チャンク 1 回分のエンコード入力 `ChunkEncodingInput` / 統一タイムライン行 `UnifiedRecentEventLine`（`application/llm/contracts/chunk_encoding.py`）
...
class SubjectiveEpisode:
    ...
    recall_text: str | None  # None 可（チャンク草案・LLM 適用前）。非空白 str のときのみ受動想起等に載せる。
```
