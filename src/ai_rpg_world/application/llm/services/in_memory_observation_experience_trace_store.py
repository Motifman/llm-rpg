...
class InMemoryObservationExperienceTraceStore(IObservationExperienceTraceStore):
    """プレイヤーごとに observation trace を保持する in-memory store。"""
    ...
    def __init__(self, max_entries_per_player: int = 1000) -> None:
        ...
        self._store: Dict[int, List[ObservationExperienceTrace]] = {}
    ...
    def append(self, player_id: PlayerId, trace: ObservationExperienceTrace) -> None:
        ...
        if not isinstance(trace, ObservationExperienceTrace):
            raise TypeError("trace must be ObservationExperienceTrace")
