...
class SpotGraphRecipientStrategy(IRecipientResolutionStrategy):
    ...
    def _deliver_market_activity(self, event: Any, add: _Add) -> None:
        ...
        居合わせた人には流さない。**同席が観測の根拠になっていない**からで、
        板がどこからでも届く世界では、出品した本人がその場に居ないのに、
        たまたま広場に居た人だけが通知を受ける。規模の話でもある — 板の全活動が
        全員へ届くと、エージェントが増えたときにプロンプトが通知で埋まる。

        出品や値の付け直しも流さない。**通知は値を運ぶ**ので、流すと板を
        引かなくても値が分かる経路が残り、板を常駐から外した意味が通知の側から
        抜ける。

        **失うものがある。** 板の動きは誰の目にも触れなくなり、「他者からの
        可視性」をここでは削ることになる。代わりに可視性を担うのは**板そのもの**
        で、引けば誰でも同じ値と直近の約定価格が読める。**押しつけられないが、
        公開されている。** 言いたい人は `say_inline` で言えるので、黙っている
        ことと言うことの差が意味を持つ。

        届けるのは相手側だけでよい。行為者は自分の手番の結果文で結果を受け取る。
        """
        if str(event.kind).startswith("expired"):
            # 期限切れの行為者は世界の時計なので、「行為者を除く」の除外が
            # 効かない (持ち主 = entity_id)。ここで明示的に持ち主へ届ける。
            add(PlayerId(int(event.entity_id)))
            return
